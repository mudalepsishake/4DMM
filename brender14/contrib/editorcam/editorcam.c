...
static br_vector3 accumulate_keys(br_uint_32 flags)
{
    ...
    /* BRenderModern: Accumulate our keys. */
...
void BrEditorCamUpdate(br_editor_camera *cam, float dt)
{
    ...
    // BRenderModern: fprintf(stderr, "xrel = %d, yrel = %d, wdiff = %f\n", xrel, yrel, wheel_diff);

    /* BRenderModern: Calculate our matrix. */
    ...
    /* BRenderModern: Scroll - discrete steps, don't dt multiply. */
