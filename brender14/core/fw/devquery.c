...
static br_boolean deviceEnum(br_device *dev, br_device_enum_cbfn *cbfn, void *args)
{
    ...
    void           *mem;
    br_token_value *tvp;
    const char     *identifier = NULL, *creator = NULL, *title = NULL, *product = NULL, *product_version = NULL;
    ...
    if(s) {
        ...
        terminate = cbfn(identifier, version, creator, title, product, product_version, args);
...
br_error BR_PUBLIC_ENTRY BrDeviceEnum(br_device_enum_cbfn *cbfn, void *args)
{
    ...
    BrSystemConfigQueryString(BRT_BRENDER_DEVICES_STR, devices_str, BR_ASIZE(devices_str));

    // BRender: If BRENDER_DEVICES is not set, use whatever value is
    // BRender: in BRENDER_DEFAULT_DEVICE instead.

    if(*devices_str == '\0') {
        BrSystemConfigQueryString(BRT_DEFAULT_DEVICE_STR, devices_str, BR_ASIZE(devices_str));
        pass_args = BR_FALSE;
    }
...
br_error BR_PUBLIC_ENTRY BrOutputFacilityEnum(const char *name, br_outfcty_enum_cbfn *cbfn, void *args)
{
    ...
    objects = BrMemAllocate(max * sizeof(br_object *), BR_MEMORY_APPLICATION);
