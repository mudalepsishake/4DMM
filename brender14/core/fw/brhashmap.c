...
/* BRenderModern: This should be optimised out in Release builds. */
...
static br_hashmap_bucket *add_or_replace_bucket(br_hashmap *hm, br_hashmap_bucket *buckets, size_t n, const br_hashmap_bucket *bkt, int *added)
{
    ...
    LOOP_BUCKETS(hm, index, bkt->hash % n)
    {
        ...
        /* BRenderModern: Shortcut: use first empty bucket. */
        ...
        /* BRenderModern: Duplicate key, replace it. */
...
int BR_RESIDENT_ENTRY BrHashMapResize(br_hashmap *hm, size_t nelem)
{
    ...
    /* BRenderModern: Nothing to do. */
    if(nelem == hm->size)
        return 0;

    /* BRenderModern: Make sure the realloc() size won't overflow. */
    if(nelem >= (SIZE_MAX / sizeof(br_hashmap_bucket)))
        return BRE_RANGE;

    /* BRenderModern: Reallocate the buckets. */
    ...
    /* BRenderModern: Shortcut - no items. no problem! */
    if(hm->size == 0) {
    ...
    /* BRenderModern: Now redistribute everything. */
    for(size_t i = 0; i < nelem; ++i) {
        int                added;
    ...
    // BRenderModern: fprintf(stderr, "Resizing to %zu\n", hm->num_buckets);
    return 0;
}
...
static int maybe_resize(br_hashmap *hm)
{
    ...
    if(hm->num_buckets >= tmp)
        return BRE_RANGE; /* BRenderModern: or EOVERFLOW? */

    /* BRenderModern: Same as "n * load_factor" */
    ...
    /* BRenderModern: Don't allow resize if explicitly disabled. */
...
int BR_RESIDENT_ENTRY BrHashMapInsert(br_hashmap *hm, const void *key, void *value)
{
    ...
    /* BRenderModern: See if we need to resize. */
...
static const br_hashmap_bucket *find_bucket(const br_hashmap *hm, const void *key, size_t *outindex)
{
    ...
    LOOP_BUCKETS(hm, index, hash % hm->num_buckets)
    {
        ...
        /* BRenderModern: Stop at first empty bucket, item isn't here. */
        if(bkt->hash == BR_INVALID_HASH)
            break;
        ...
        /* BRenderModern: In case of hash collision. */
...
br_boolean BR_RESIDENT_ENTRY BrHashMapUpdate(br_hashmap *hm, const void *key, void *value)
{
    ...
    /* BRenderModern: NB: Safe const-away cast. */
...
void *BR_RESIDENT_ENTRY BrHashMapRemove(br_hashmap *hm, const void *key)
{
    ...
    /* BRenderModern: Search through the remaining buckets, to see if we can fill the gap. */
    LOOP_BUCKETS(hm, cidx, index + 1)
    {
        size_t             nidx;
        br_hashmap_bucket *bkt2 = hm->buckets + cidx;

        /* BRenderModern: Have an empty bucket, we're done here! */
        if(bkt2->hash == BR_INVALID_HASH)
            break;

        /* BRenderModern: If our hash can be moved back, do it. */
