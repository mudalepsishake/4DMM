...
static const char *const DriverExtensions[] = {
    ".bdd", /* BRenderModern: BRender Device Driver        */
    ".bed", /* BRenderModern: BRender Device Driver (Eval) */
    ".dll", /* BRenderModern: BRender Device Driver        */
};
#else
static const char *const DriverExtensions[] = {
    ".sobdd", /* BRenderModern: Linux/Unix BRender Device Driver */
    ".so",    /* BRenderModern: Linux/Unix BRender Device Driver */
};
#endif
...
br_image *BR_RESIDENT_ENTRY BrImageReference(const char *name)
{
     ...
     */
    if(img) {
        /* BRender: Splat any extension from the identifier to avoid problems when searching for loaded image.  */
        char *tmp;
