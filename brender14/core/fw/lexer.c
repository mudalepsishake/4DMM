    ...
    for(;;) {
        ...
        if(l->source->next == BR_EOF) {
            ...
            for(n = 1; n < l->string_buffer_size - 1;) {
                l->source->getchar(l->source);
                if(!(BrIsAlpha(l->source->next) || BrIsDigit(l->source->next) || l->source->next == '_'))
                    break;
                l->string_buffer[n++] = (char)l->source->next;
            }
            l->string_buffer[n] = '\0';
            ...
            for(n = 0; n < l->nkeywords; n++) {
                if(l->keywords[n].name[0] == l->string_buffer[0] && !BrStrICmp(l->keywords[n].name, l->string_buffer)) {
                    /* BRender:
            ...
            if(got_point) {
                ...
                if(n >= 3 && (l->string_buffer[0] == '0' && (l->string_buffer[1] == 'x' || l->string_buffer[1] == 'X'))) {
                    if(BrSScanf(l->string_buffer + 2, "%" SCNxPTR, &l->current.v.integer) != 1)
                        l->current.v.integer = 0;
                } else {
                    if(BrSScanf(l->string_buffer, "%" SCNdPTR, &l->current.v.integer) != 1)
                        l->current.v.integer = 0;
                }
                // BRenderModern: l->current.v.integer = BrStrToL(l->string_buffer,0,0);
