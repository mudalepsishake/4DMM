...
static wchar_t *BrStdioToWinWchar(void *vres, const char *s)
{
    ...
    /* BRenderModern: Get the required size. */
 ...
 * if open failed
 */
static void *BR_CALLBACK BrStdioOpenRead(const char *name, br_size_t n_magics, br_mode_test_cbfn *identify, int *mode_result)
{
    FILE     *fh;
