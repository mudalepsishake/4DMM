...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, doubleBuffer)(br_device_pixelmap *self, br_device_pixelmap *src)
{
    ...
    // BRender:	return DevicePixelmapCopy(self, src);
}

br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, copyDirty)(br_device_pixelmap *self, br_device_pixelmap *src, br_rectangle *dirty, br_int_32 num_rects)
{
    int      i;
    br_point p;
    br_error e;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, copyToDirty)(br_device_pixelmap *self, br_device_pixelmap *src, br_rectangle *dirty,
                                                              br_int_32 num_rects)
{
    int      i;
    br_point p;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, copyFromDirty)(br_device_pixelmap *self, br_device_pixelmap *src, br_rectangle *dirty,
                                                                br_int_32 num_rects)
{
    int      i;
    br_point p;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, fillDirty)(br_device_pixelmap *self, br_uint_32 colour, br_rectangle *dirty, br_int_32 num_rects)
{
    int      i;
    br_error e;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, doubleBufferDirty)(br_device_pixelmap *self, br_device_pixelmap *src, br_rectangle *dirty,
                                                                    br_int_32 num_rects)
{
    int      i;
    br_point p;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, rectangle)(br_device_pixelmap *self, br_rectangle *rect, br_uint_32 colour)
{
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, rectangle2)(br_device_pixelmap *self, br_rectangle *rect, br_uint_32 colour_tl, br_uint_32 colour_br)
{
    br_point tl, tr, bl, br;
    br_error e;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, line)(br_device_pixelmap *self, br_point *s, br_point *e, br_uint_32 colour)
{
...
#define absolute(i, j, k) ((i - j) * (k = ((i - j) < 0 ? -1 : 1)))
    ...
    /* BRender: decide increment sign by the slope sign */
    if(sign_x == sign_y)
        step = 1;
    else
        step = -1;

    if(dy > dx) { /* BRender: chooses axis of greatest movement (make dx) */
        swap(as.x, as.y);
        swap(ae.x, ae.y);
        swap(dx, dy);
        reverse = 1;
    ...
    /* BRender: note error check for dx==0 should be included here */
    if(as.x > ae.x) { /* BRender: start from the smaller coordinate */
        x  = ae.x;
        y  = ae.y;
        x1 = as.x;
        y1 = as.y;
    ...
    /* BRender: Note dx=n implies 0 - n or (dx+1) pixels to be set */
    /* BRender: Go round loop dx/4 times then plot last 0,1,2 or 3 pixels */
    /* BRender: In fact (dx-1)/4 as 2 pixels are already plottted */

    xend        = (dx - 1) / 4;
    pixels_left = (dx - 1) % 4; /* BRender: number of pixels left over at the end */
    LINEPLOT(x, y, reverse);
    LINEPLOT(x1, y1, reverse); /* BRender: plot first two points */
    incr2 = 4 * dy - 2 * dx;
    if(incr2 < 0) { /* BRender: slope less than 1/2 */
        c     = 2 * dy;
        incr1 = 2 * c;
        D     = incr1 - dx;

        for(i = 0; i < xend; i++) { /* BRender: plotting loop */
            ++x;
            --x1;
            if(D < 0) {
                /* BRender: pattern 1 forwards */
                LINEPLOT(x, y, reverse);
                LINEPLOT(++x, y, reverse);
                /* BRender: pattern 1 backwards */
                LINEPLOT(x1, y1, reverse);
                LINEPLOT(--x1, y1, reverse);
                D += incr1;
            } else {
                if(D < c) {
                    /* BRender: pattern 2 forwards */
                    LINEPLOT(x, y, reverse);
                    LINEPLOT(++x, y += step, reverse);
                    /* BRender: pattern 2 backwards */
                    LINEPLOT(x1, y1, reverse);
                    LINEPLOT(--x1, y1 -= step, reverse);
                } else {
                    /* BRender: pattern 3 forwards */
                    LINEPLOT(x, y += step, reverse);
                    LINEPLOT(++x, y, reverse);
                    /* BRender: pattern 3 backwards */
                    LINEPLOT(x1, y1 -= step, reverse);
                    LINEPLOT(--x1, y1, reverse);
                }
                D += incr2;
        ...
        /* BRender: plot last pattern */
        if(pixels_left) {
            if(D < 0) {
                LINEPLOT(++x, y, reverse); /* BRender: pattern 1 */
                if(pixels_left > 1)
                    LINEPLOT(++x, y, reverse);
                if(pixels_left > 2)
                    LINEPLOT(--x1, y1, reverse);
                ...
                if(D < c) {
                    LINEPLOT(++x, y, reverse); /* BRender: pattern 2  */
                    if(pixels_left > 1)
                        LINEPLOT(++x, y += step, reverse);
                    if(pixels_left > 2)
                        LINEPLOT(--x1, y1, reverse);
        ...
        } /* BRender: end if pixels_left */
    }
    /* BRender: end slope < 1/2 */
    else { /* BRender: slope greater than 1/2 */
        c     = 2 * (dy - dx);
        incr1 = 2 * c;
        D     = incr1 + dx;
        for(i = 0; i < xend; i++) {
            ...
            --x1;
            if(D > 0) {
                /* BRender: pattern 4 forwards */
                LINEPLOT(x, y += step, reverse);
                LINEPLOT(++x, y += step, reverse);
                /* BRender: pattern 4 backwards */
                LINEPLOT(x1, y1 -= step, reverse);
                LINEPLOT(--x1, y1 -= step, reverse);
                D += incr1;
            } else {
        ...
        if(pixels_left) {
            if(D > 0) {
                LINEPLOT(++x, y += step, reverse); /* BRender: pattern 4 */
                if(pixels_left > 1)
                    LINEPLOT(++x, y += step, reverse);
                if(pixels_left > 2)
                    LINEPLOT(--x1, y1 -= step, reverse);
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, text)(br_device_pixelmap *self, br_point *point, br_font *font, const char *text, br_uint_32 colour)
{
    br_rectangle r;
    br_point     p;
    br_int_32    x, y;
    ...
    if(font->flags & BR_FONTF_PROPORTIONAL) {
        ...
        for(; *(unsigned char *)text; text++) {
            ...
            if(x + r.w > 0)
                DevicePixelmapCopyBits(self, &p, font->encoding[*(unsigned char *)text], (br_uint_16)s_stride, &r, colour);
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, textBounds)(br_device_pixelmap *self, br_rectangle *rect, br_font *font, const char *text)
{
    int i, j;
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, copyBits)(br_device_pixelmap *self, br_point *point, const br_uint_8 *src,
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_gen, originSet)(br_device_pixelmap *self, br_point *p)
{
    self->pm_origin_x = (br_int_16)p->x;
    self->pm_origin_y = (br_int_16)p->y;
    return BRE_OK;
}
