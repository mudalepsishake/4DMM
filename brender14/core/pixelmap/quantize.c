...
struct box {
    int r0; /* BRender: min value, exclusive */
    int r1; /* BRender: max value, inclusive */
    int g0;
    int g1;
    int b0;
    int b1;
...
/* BRender: Histogram is in elements 1..HISTSIZE along each axis,
...
/* BRender: We now convert histogram into moments so that we can rapidly calculate
...
static void Moments(void)
{
    ...
    for(r = 1; r <= 32; ++r) {
        ...
        for(g = 1; g <= 32; ++g) {
            ...
            for(b = 1; b <= 32; ++b) {
                ind1 = (r << 10) + (r << 6) + r + (g << 5) + g + b; /* BRender: [r][g][b] */
                line += ((long *)wt)[ind1];
                line_r += ((long *)mr)[ind1];
                line_g += ((long *)mg)[ind1];
                line_b += ((long *)mb)[ind1];
                ...
                ind2 = ind1 - 1089; /* BRender: [r-1][g][b] */
...
static int Cut(struct box *set1, struct box *set2)
{
    ...
    if((maxr >= maxg) && (maxr >= maxb)) {
        dir = RED;
        if(cutr < 0)
            return 0; /* BRender: can't split the box */
    } else {
...
void BR_PUBLIC_ENTRY BrQuantMakePalette(int base, int num_entries, br_pixelmap *palette)
{
    ...
    for(i = 1; i < K; ++i) {
        if(Cut(&cube[next], &cube[i])) {
            ...
            vv[next] = 0.0F; /* BRender: don't try to split this box again */
            i--;             /* BRender: didn't create box i */
        }
