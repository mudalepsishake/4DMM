                                                  ...
                                                  br_uint_32 range, br_uint_32 quantizationMethod)
{
    ...
    br_colour    colour;
    void        *destPtr;
    double       r[256], g[256], b[256], red, green, blue, y, i, q; // BRender: r=y g=i b=q for yiq quantization
    unsigned     row, col;
    int          area;
    unsigned     lowest;
    ...
    UASSERT(destinationType == BR_PMT_INDEX_8 ? !!palette : BR_TRUE);
    ...
    // BRender: Build representation of palette in form appropriate for quantization.
    ...
    if(destinationType == BR_PMT_INDEX_8) {
        switch(quantizationMethod) {
            ...
            default: // BRender: BR_QUANTTIZE_RGB
                for(col = 0; col < 256; col++) {
                    colour = BrPixelmapPixelGet(palette, 0, col);
                    r[col] = BR_RED(colour);
                    g[col] = BR_GRN(colour);
    ...
    // BRender: Build first level of agregate map
    ...
    // BRender: Build first mip level (a simple conversion)
        ...
        for(col = 0; col < (unsigned)SQ(destination->width); col++) {
            ...
            switch(destination->type) {
                ...
                default:
                    ...
                    lowest      = base;
                    lowestValue = 1000000000.0; // BRender: large value
    ...
    // BRender: Build subsequent levels
