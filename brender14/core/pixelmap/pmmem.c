...
struct pm_type_info pmTypeInfo[] = {
    ...
    /** BRender: ***These may be wrong and need updating. *****/
    ...
    /* BRender: Accelerator extensions. */
...
static struct br_tv_template_entry devicePixelmapTemplateEntries[] = {
    ...
    // BRenderModern: {BRT_PIXEL_CHANNELS_I32, NULL, 0,                BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_PIXEL_CHANNELS_TL,  NULL, 0,                BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_INDEXED_B,          NULL, F(pm_indexed),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_ORIGIN_V2_I,        NULL, F(pm_indexed),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_PIXEL_BITS_I32,     NULL, F(pm_indexed),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_WORD_BYTES_I32,     NULL, F(pm_indexed),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
    // BRenderModern: {BRT_MEMORY_MAPPED,      NULL, F(pm_indexed),    BRTV_QUERY | BRTV_ALL, BRTV_CONV_CUSTOM },
};
// BRenderModern: clang-format on
#undef F
...
br_error BR_CMETHOD_DECL(br_device_pixelmap_mem, match)(br_device_pixelmap *self, br_device_pixelmap **newpm, br_token_value *tv)
{
    ...
    switch(mt.use) {
        ...
        case BRT_CLONE:
            if((*newpm = (br_device_pixelmap *)BrPixelmapConvert((br_pixelmap *)self, mt.pixel_type, NULL)) == NULL)
                return BRE_FAIL;
...
// BRender: absoloute value for an integer
#define I_ABS(x) ((x) >= 0 ? (x) : -(x))
// BRender: positive part of an integer
#define I_POS(x) ((x) >= 0 ? (x) : 0)
// BRender: negative part of an integer
#define I_NEG(x) ((x) <= 0 ? (x) : 0)
// BRender: absoloute value of negative part of an integer
...
// BRender: positive values encroach onto image, negative define a border around it.
