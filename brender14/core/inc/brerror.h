...
#define BRE_DEV_FAIL                 BR_ERROR_MAKE(BR_ECLASS_DEVICE, 1) // BRender: All general device errors
#define BRE_DEV_HARDWARE_NOT_PRESENT BR_ERROR_MAKE(BR_ECLASS_DEVICE, 2) // BRender: Hardware not detected: usually device library failed init
#define BRE_DEV_NO_MEMORY            BR_ERROR_MAKE(BR_ECLASS_DEVICE, 3) // BRender: Driver failed in mem allocate
#define BRE_DEV_NO_VIDEO_MEMORY      BR_ERROR_MAKE(BR_ECLASS_DEVICE, 4) // BRender: Driver failed in video mem allocate
#define BRE_DEV_UNSUPPORTED          BR_ERROR_MAKE(BR_ECLASS_DEVICE, 5) // BRender: Device does not support capabilities
#define BRE_DEV_HARDWARE_SET         BR_ERROR_MAKE(BR_ECLASS_DEVICE, 6) // BRender: Driver failed some hardware setup call
#define BRE_DEV_HARDWARE_NOT_READY   BR_ERROR_MAKE(BR_ECLASS_DEVICE, 7)
#define BRE_DEV_NOT_FOUND            BR_ERROR_MAKE(BR_ECLASS_DEVICE, 8) // BRender: Device driver not present (BRender should return this)
#define BRE_DEV_REFUSED              BR_ERROR_MAKE(BR_ECLASS_DEVICE, 9)  // BRender: Some request refused
#define BRE_DEV_LOCK_FAIL            BR_ERROR_MAKE(BR_ECLASS_DEVICE, 10) // BRender: Some locking call failed
#define BRE_DEV_OUTPUT_UNAVAILABLE   BR_ERROR_MAKE(BR_ECLASS_DEVICE, 11) // BRender: Output facility is not available
#define BRE_DEV_ALREADY_INIT         BR_ERROR_MAKE(BR_ECLASS_DEVICE, 12) // BRender: Some facility is already initialised
#define BRE_DEV_DIRECTDRAW_ERROR     BR_ERROR_MAKE(BR_ECLASS_DEVICE, 13) // BRender: Direct Draw returned an error not covered by other codes
// BRenderModern: clang-format on

/* BRenderModern: error class structure for printing error strings */
