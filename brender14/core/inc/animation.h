...
typedef struct br_animation_channel {
    br_int_32  target;        /* BRenderModern: index into owning br_animation_set actors */
    br_int_32  path;          /* BRenderModern: BR_ANIM_PATH_* or -1 to skip */
    br_int_32  interpolation; /* BRenderModern: BR_ANIM_INTERP_* */
    br_int_32  nkeys;         /* BRenderModern: no. keys */
    br_scalar *times;         /* BRenderModern: [nkeys] */
    br_scalar *values;        /* BRenderModern: [nkeys * (path==ROTATION ? 4 : 3)] */
...
typedef struct br_animation_set {
    ...
    char              *identifier;
    br_int_32          nactors;
    br_animation_pose *rest_poses; /* BRenderModern: [nactors] */
    br_animation_clip *clips;      /* BRenderModern: [nclips] */
    br_int_32          nclips;
} br_animation_set;
...
typedef struct br_animation_instance {
    ...
    br_actor        **actors;    /* BRenderModern: weak refs [nactors] */
    br_int_32         nactors;   /* BRenderModern: must equal set->nactors */
    br_int_32         active;    /* BRenderModern: clip index (0..nclips-1), or -1 for all */
    br_scalar        *_eval_buf; /* BRenderModern: internal: lazily allocated working TRS buffer */
