                           ...
                           br_scalar origin_y)
{
    ...
    UASSERT(hither <= BR_SCALAR(0.0)); /* BRender: hither and yon are in view space - view volume is along -ve Z */
...
br_token CameraToScreenMatrix4(br_matrix4 *mat, br_actor *camera)
{
    ...
    switch(camera_type->type) {
        ...
        case BR_CAMERA_PARALLEL:
            ...
            BrMatrix34Scale(&mat34, BR_CONST_MUL(BR_RCP(BR_MUL(camera_type->width, camera_type->aspect)), 2),
