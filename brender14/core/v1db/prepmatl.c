...
void BR_PUBLIC_ENTRY BrMaterialUpdate(br_material *mat, br_uint_16 flags)
{
    ...
    if(v1db.renderer == NULL) {
        ...
        /*
         * v242: `stored` is renderer-private. A material registered before the
         * first renderer cannot legitimately own a stored renderer state.
         */
        mat->stored = NULL;
#endif
        return;
    ...
    if(flags & BR_MATU_RENDERING) {
        ...
        tvp->v.b = !!(mat->flags & (BR_MATF_LIGHT | BR_MATF_PRELIT)) || mat->colour != BR_COLOUR_RGB(255, 255, 255);
        tvp++;

        tvp->t   = BRT_BLEND_B;
        tvp->v.b = (mat->opacity < 255) || mat->flags & (BR_MATF_PREALPHA | BR_MATF_BLEND);
        tvp++;
    ...
    if(flags & BR_MATU_COLOURMAP) {
        ...
        tvp->v.o = mat->colour_map ? (br_object *)BrBufferStoredGet(mat->colour_map) : NULL;
        tvp++;

        tvp->t   = BRT_INDEX_SHADE_O;
        tvp->v.o = mat->index_shade ? (br_object *)BrBufferStoredGet(mat->index_shade) : NULL;
        tvp++;

        tvp->t   = BRT_INDEX_BLEND_O;
        tvp->v.o = mat->index_blend ? (br_object *)BrBufferStoredGet(mat->index_blend) : NULL;
        tvp++;

        tvp->t   = BRT_INDEX_FOG_O;
        tvp->v.o = mat->index_fog ? (br_object *)BrBufferStoredGet(mat->index_fog) : NULL;
        tvp++;
    ...
    if(flags & BR_MATU_SCREENDOOR) {
        ...
        tvp->v.o = mat->screendoor ? (br_object *)BrBufferStoredGet(mat->screendoor) : NULL;
        tvp++;
