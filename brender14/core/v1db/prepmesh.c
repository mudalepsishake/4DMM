 ...
 */
struct pm_temp_edge {
    struct pm_temp_edge *next;  /* BRender: next in chain */
    br_uint_16           first; /* BRender: First vertex */
    br_uint_16           last;  /* BRender: Last Vertex */
    char                 other; /* BRender: Edge is used in other direction */
};
...
static void prepareEdges(struct v11group *group, br_model *model)
{
    ...
    br_face  *mfp;
    // BRender:	struct v11face *fp;
    br_vector3_u16 *vertex_numbers;
    br_vector3_u16 *edges;
    // BRender:	br_vector4 * plane_equation;

    int f;

    scratch_size = group->nfaces * 3 * sizeof(*pm_edge_table) + group->nvertices * sizeof(*pm_edge_hash);
...
static void PrepareFaceNormals(br_model *model)
{
    ...
    for(f = 0; f < model->nfaces; f++, fp++) {
        ...
        BrPlaneEquation(&v4, &vertices[fp->vertices[0]].p, &vertices[fp->vertices[1]].p, &vertices[fp->vertices[2]].p);
 ...
 */
static void Smoothing(br_model *model, br_scalar crease_limit, struct prep_vertex **start, struct prep_vertex **end)
{
    struct prep_vertex **outer, **inner;

    for(outer = start; outer < end; outer++) {

#if BRENDER_BUILD_FOR_CROC // BRenderModern: Kludging flat shading for Croc
        /* BRender:
 ...
 */
static void SmoothingCreased(br_model *model, br_scalar crease_limit, struct prep_vertex **start, struct prep_vertex **end)
{
    br_vector3           o_n;
    struct prep_vertex **outer, **inner;
...
static void PrepareGroups(br_model *model)
{
    ...
    // BRender:	struct v11face *v11f;
    br_vector3_u16 *vertex_numbers;
    br_vector3_u16 *edges;
    br_vector4     *eqn;
    // BRender:	struct fmt_vertex *v11v;
    br_vector3 *position;
    br_vector2 *map;
    br_vector3 *normal;
    br_fvector3 last_n;
    ...
    // BRender:	v11f = (void *)cp; cp+= PREP_ALIGN(nf * sizeof(struct v11face));
    vertex_numbers = (void *)cp;
    cp += PREP_ALIGN(nf * sizeof(*v11g->vertex_numbers));
    edges = (void *)cp;
    cp += PREP_ALIGN(nf * sizeof(*v11g->edges));
    eqn = (void *)cp;
    cp += PREP_ALIGN(nf * sizeof(*v11g->eqn));

    // BRender:	v11v = (void *)cp; cp+= PREP_ALIGN(nv * sizeof(struct fmt_vertex));
    position = (void *)cp;
    cp += PREP_ALIGN(nv * sizeof(*v11g->position));
    map = (void *)cp;
    cp += PREP_ALIGN(nv * sizeof(*v11g->map));
    normal = (void *)cp;
    cp += PREP_ALIGN(nv * sizeof(*v11g->normal));
    ...
    v11fflags = (void *)cp;
    cp += PREP_ALIGN(nf * sizeof(br_uint_8));
    ...
    for(v = 0, g = 0, count = 0; v < ntemps - 1; v++) {
        ...
        if(model->faces[sorted_vertices[v]->f].material != model->faces[sorted_vertices[v + 1]->f].material) {
            g++;
            // BRender:			v11g[g].vertices = v11v+count+1;
...
void RegenerateVertexNormals(struct v11model *v11m)
{
    ...
    for(g = 0; g < v11m->ngroups; g++) {
         ...
         */
        // BRender:		fp=v11m->groups[g].faces;
        vertex_numbers = v11m->groups[g].vertex_numbers;
        eqn            = v11m->groups[g].eqn;
...
void BR_PUBLIC_ENTRY BrModelUpdate(br_model *model, br_uint_16 flags)
{
    ...
    /*
     * v242: `stored` belongs to a live renderer. A model prepared or registered
     * before the first renderer cannot legitimately retain one.
     */
    ...
    if(model->faces == NULL || model->vertices == NULL)
        BR_FAILURE1("BrModelUpdate: model has no faces or vertices (%s)", model->identifier ? model->identifier : "<NULL>");

    v11m = model->prepared;
    if(v11m) {
        // BRender: initialise the centre and radius to the old centre and radius
        centre         = v11m->centre;
        centred_radius = v11m->centred_radius;
        centre_valid   = BR_TRUE; // BRender: assume the old radius is either correct or will be overwritten with a correct value
    }
    ...
    if(!(model->flags & BR_MODF_CUSTOM_BOUNDS) && (flags & (BR_MODU_VERTICES | BR_MODU_VERTEX_POSITIONS))) {
        ...
        // BRenderModern: use the average of the bounds as an approximation to the centre of the model (that gives the minimal bounding
        // BRenderModern: radius)
     ...
     */
    if(flags & _BR_MODU_RESERVED && !(model->flags & BR_MODF_USED_PREPARED_USER)) {
        ...
        if(model->vertices && (flags & MODU_VERTEX_COPY_FLAGS)) {
            for(g = 0; g < v11m->ngroups; g++) {
                for(v = 0; v < v11m->groups[g].nvertices; v++) {
                    // BRender:					fvp = v11m->groups[g].vertices+v;
...
#if !BRENDER_BUILD_FOR_CROC // BRenderModern: Disabled for Croc - it doesn't work right anyhow!
            if(!(model->flags & BR_MODF_CUSTOM_NORMALS))
