        ...
        if(fp->material) {
            for(j = 0; j < nmaterials; j++) {
                if(mindex[j] == fp->material) {
                    *ip = j + 1; /* BRender: Index entries start at 1 */
                    break;
                }
            }
        }
...
br_uint_32 BR_PUBLIC_ENTRY BrModelSaveMany(const char *filename, br_model **models, br_uint_16 num)
{
    ...
    // BRender: Only updateable models can be written at present,
    // BRender: so check the first one
...
static int WriteActor(br_actor *a, br_datafile *df)
{
    ...
    // BRender: No children ? We're done ...
    ...
    // BRender: For the children to retain the same order when reloaded, we need
    // BRender: to save them out in reverse order

    ap = a->children;

    // BRender: Find last child in the list ...
    ...
    // BRender: Write out list in reverse ... stop when we get back to the parent
