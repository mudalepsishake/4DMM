 ...
 */
static void InsertSortOrderTablePrimitive(br_primitive **bucket, br_primitive *p)
{
    br_primitive *cur, *prev;
    ...
    for(cur = *bucket, prev = NULL; cur != NULL;) {
        ASSERT(cur->next != cur);

        if(cur->depth >= p->depth) {
            p->next = cur;
    ...
    /* BRenderModern:
     * Not inserted, dump us at the end
     */
