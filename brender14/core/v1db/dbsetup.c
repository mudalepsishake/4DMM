...
/*
 * v242: 3DMM can register legacy/application-owned render resources before the
 * first renderer exists. Their BRender 1.4 `stored` fields are renderer-private
 * and cannot contain a valid object in that state. Some movie-owned resources
 * reuse or overwrite those legacy structure bytes after registration, so merely
 * clearing `stored` when Br*Update() first runs is not enough.
 *
 * Immediately before the first renderer adopts the registries, discard every
 * pre-renderer stored value without dispatching through it. The normal update
 * pass below then creates fresh objects owned by this renderer.
 */
static br_uint_32 BR_CALLBACK discardColdBufferStored(br_pixelmap *item, void *arg)
{
    ASSERT_MESSAGE("BR_CALLBACK discardColdBufferStored NULL pointer", item != NULL);
    item->stored = NULL;
    return 0;
}

static br_uint_32 BR_CALLBACK discardColdMaterialStored(br_material *item, void *arg)
{
    ASSERT_MESSAGE("BR_CALLBACK discardColdMaterialStored NULL pointer", item != NULL);
    item->stored = NULL;
    return 0;
}

static br_uint_32 BR_CALLBACK discardColdModelStored(br_model *item, void *arg)
{
    ASSERT_MESSAGE("BR_CALLBACK discardColdModelStored NULL pointer", item != NULL);
    item->stored = NULL;
    return 0;
}
#endif
...
br_error BR_PUBLIC_ENTRY BrV1dbRendererBegin(br_device_pixelmap *destination, br_renderer *renderer)
{
    ...
    if(renderer == NULL) {
        ...
        if(destination != NULL) {
            /* BRenderModern:
             * If a destination pixelmap is given, pass it on to the new method
...
#if BRENDER_LEGACY_3DMM_MODEL_ABI
    /*
     * Renderer-private state from before this first renderer is never valid.
     * Clear it as one atomic cold-start boundary before any registry item is
     * updated or queried through an object dispatch table.
     */
    BrTableEnum(NULL, discardColdBufferStored, NULL);
    BrMapEnum(NULL, discardColdBufferStored, NULL);
    BrMaterialEnum(NULL, discardColdMaterialStored, NULL);
    BrModelEnum(NULL, discardColdModelStored, NULL);
#endif
    ...
    // BRender: STEVE : For some bizzare reason that I don't have time to look
    // BRender: at now, we need to do two material updates on the default material
    // BRender: to get it to work. Weird.
...
{
    if(v1db.zb_active && v1db.zs_active)
        BR_ERROR0("Invalid BrZbEnd() call, please use BrRendererEnd() instead");
...
{
    if(v1db.zb_active && v1db.zs_active)
        BR_ERROR0("Invalid BrZsEnd() call, please use BrRendererEnd() instead");
