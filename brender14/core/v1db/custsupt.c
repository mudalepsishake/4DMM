 ...
 */
#define PERSP_DIV_Z(a, b) (-32767.0f * (a) / (b))
...
void BR_PUBLIC_ENTRY BrModelToScreenQuery(br_matrix4 *dest)
{
    ...
    UASSERT_MESSAGE("BrModelToScreenQuery NULL pointer to the destination matrix that holds the current "
...
void BR_PUBLIC_ENTRY BrModelToViewQuery(br_matrix34 *dest)
{
    ...
    UASSERT(v1db.rendering);
    ASSERT(v1db.renderer);
    UASSERT_MESSAGE("BrModelToViewQuery NULL pointer to the destination matrix that holds the current model_to_view "
                    "transform",
                    dest != NULL);
...
{
    UASSERT(v1db.rendering);
    UASSERT_MESSAGE("BrOriginToScreenXY NULL pointer to the destination vector to receive the coords.in projected "
                    "screen space",
                    screen != NULL);
...
br_uint_32 BR_PUBLIC_ENTRY BrOriginToScreenXYZO(br_vector3 *screen)
{
    ...
    UASSERT(v1db.rendering);
    UASSERT_MESSAGE("BrOriginToScreenXYZO NULL pointer to the destination vector to receive the coords.in projected "
                    "screen space",
                    screen != NULL);
...
br_uint_32 BR_PUBLIC_ENTRY BrPointToScreenXYZO(br_vector3 *screen, br_vector3 *point)
{
    ...
    UASSERT(v1db.rendering);
    UASSERT_MESSAGE("BrPointToScreenXYZO NULL pointer to the destination vector to receive the coords.in projected "
                    "screen space",
                    screen != NULL);
    UASSERT_MESSAGE("BrPointToScreenXYZO NULL pointer to the source vector containing the coords of the point to "
                    "project",
                    point != NULL);
...
void BR_PUBLIC_ENTRY BrPointToScreenXYMany(br_vector2 *screens, br_vector3 *points, br_uint_32 npoints)
{
    ...
    UASSERT_MESSAGE("BrPointToScreenXYMany NULL pointer to an array of source vectors containing the coords of the "
...
{
    UASSERT_MESSAGE("BrZbDepthToScreenZ NULL pointer to camera being used for rendering",
                    camera != NULL); // BRender: Not accessed, but should still be valid
...
br_uint_32 BR_PUBLIC_ENTRY BrZbScreenZToDepth(br_scalar sz, const br_camera *camera)
{
    ...
    UASSERT_MESSAGE("BrZbScreenZToDepth NULL pointer to camera being used for rendering",
                    camera != NULL); // BRender: Not accessed, but should still be valid
                                     /* BRender: Not accessed, but should still be valid */
#if 1
    if(sz <= TOONEAR)
        return 0UL;
    else if(sz >= TOOFAR)
        return 0xffffffffUL;
#else /* BRender: Alternative that caters for border line cases - if they can occur */
    if(sz < NEAREST || sz > FARTHEST) /* BRender: Only do further processing if screen z outside normal range */
    {
        if(sz < NEAREST) {
            if(sz < TOONEAR || depth > BR_FIXED_SIGN) /* BRender: Either a long way off or just beyond the border line */
                return 0UL;
        } else {
            if(sz > TOOFAR || depth < BR_FIXED_SIGN) /* BRender: Either a long way off or just beyond the border line */
                return 0xffffffffUL;
        }
    }
#endif
...
br_scalar BR_PUBLIC_ENTRY BrZsDepthToScreenZ(br_scalar depth_z, const br_camera *camera)
{
    ...
    yon    = camera->yon_z;
    UASSERT_MESSAGE("BrZsDepthToScreenZ NULL pointer to camera being used for rendering",
                    camera != NULL); // BRender: Not accessed, but should still be valid
...
br_scalar BR_PUBLIC_ENTRY BrZsScreenZToDepth(br_scalar sz, const br_camera *camera)
{
    ...
    hither = camera->hither_z;
    yon    = camera->yon_z;
    UASSERT_MESSAGE("BrZsScreenZToDepth NULL pointer to camera being used for rendering",
                    camera != NULL); // BRender: Not accessed, but should still be valid

    /* BRender:	depth=BR_CONST_DIV(BR_MULDIV(sz,yon+hither,BR_SCALAR(-32768.f))-yon+hither,2); Not so good */
...
br_scalar BR_PUBLIC_ENTRY BrScreenZToCamera(const br_actor *camera, br_scalar sz)
{
    ...
    switch(data->type) {
        case BR_CAMERA_PARALLEL:
            return BR_SUB(BR_MULDIV(BR_CONST_DIV(sz, 2), BR_CONST_DIV(BR_SUB(hither, yon), 2), BR_SCALAR(16384)),
                                         ...
                                         br_int_16 y, br_scalar sz)
{
    ...
    vz = BrScreenZToCamera(camera, sz); // BRender: Get the original view Z
    ...
    hx = BR_DIV(BrIntToScalar(screen_buffer->width - 2 * (x + screen_buffer->origin_x)),
                BrIntToScalar(screen_buffer->width)); // BRender: homogenous -x	[ hx=2(sx+ox)/sw-1, -hx=(sw-2(sx+ox))/sw ]
    hy = BR_DIV(BrIntToScalar(2 * (y + screen_buffer->origin_y) - screen_buffer->height),
                BrIntToScalar(screen_buffer->height)); // BRender: homogenous -y	[ hy=1-2(sy+oy)/sh, -hy=(2(sy+oy)-sh)/sh ]

    switch(data->type) {
        case BR_CAMERA_PARALLEL:
            point->v[0] = BR_CONST_DIV(BR_MUL(BR_NEG(hx), BR_MUL(data->width, data->aspect)), 2); // BRender: Vx=-hx*cw*ca/2
            point->v[1] = BR_CONST_DIV(BR_MUL(BR_NEG(hy), data->height), 2);                      // BRender: Vy=-hy*ch/2
            point->v[2] = vz;                                                                     // BRender: Vz
            break;
        case BR_CAMERA_PERSPECTIVE:
            fov = (br_angle)(data->field_of_view / 2);

            UASSERT_MESSAGE("BrScreenXYZToCamera - Divide by zero error", BR_SIN(fov) != 0);
            scale = BR_DIV(BR_COS(fov),
                           BR_SIN(fov)); // BRender: Refer to BrMatrix4Perspective() - we are using its terms in reverse

            UASSERT_MESSAGE("BrScreenXYZToCamera - Divide by zero error", scale != 0);
            point->v[0] = BR_MULDIV(BR_MUL(hx, data->aspect), vz, scale); // BRender: View x
            point->v[1] = BR_MULDIV(hy, vz, scale);                       // BRender: View y
            point->v[2] = vz;                                             // BRender: View z
            break;
        default:
            UASSERT(0);
            break;
