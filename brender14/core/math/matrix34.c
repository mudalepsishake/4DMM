...
void BR_PUBLIC_ENTRY BrMatrix34Scale(br_matrix34 *mat, br_scalar sx, br_scalar sy, br_scalar sz)
{
    ...
    // BRenderModern: clang-format off
    M(0, 0) = sx; M(0, 1) = S0; M(0, 2) = S0; M(1, 0) = S0;
    M(1, 1) = sy; M(1, 2) = S0; M(2, 0) = S0; M(2, 1) = S0;
    M(2, 2) = sz; M(3, 0) = S0; M(3, 1) = S0; M(3, 2) = S0;
    // BRenderModern: clang-format on
}
...
br_matrix34 *BR_PUBLIC_ENTRY BrMatrix34FromTRS(br_matrix34 *mat, const br_vector3 *t, const br_quat *r, const br_vector3 *s)
{
    ...
    /* BRenderModern: Scale each row by the corresponding component */
