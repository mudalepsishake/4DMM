...
/* BRenderModern: Defined in animgltf.c */
...
typedef struct br_gltf_load_state {
    ...
    const char            *base_path; /** BRenderModern: < Directory for resolving relative URIs (with trailing slash). */

    br_actor **all_actors; /** BRenderModern: < All actors in the same order as the file. */
} br_gltf_load_state;
...
static br_pixelmap *load_pixelmap(br_gltf_load_state *state, const cgltf_image *image)
{
    ...
    tmp->identifier = (char *)image->name; /* BRenderModern: NB: This is safe, the clone below will copy it. */
...
static br_actor *create_empty_actor(const cgltf_node *node)
{
    ...
    if(node->brender_light != NULL) {
        /* BRenderModern: FIXME: Do I handle regular lights too? */
    ...
    switch(camera->type) {
        ...
        case cgltf_camera_type_perspective:
            ...
            if(camera->data.perspective.has_zfar) {
                ...
                // BRenderModern: FIXME: this good enough?
...
br_fmt_results *BR_PUBLIC_ENTRY BrFmtGLTFActorLoadMany(const char *name, const br_gltf_options *options)
{
     ...
     * Allocate things.
     */
    results = BrResAllocate(state, sizeof(br_fmt_results), BR_MEMORY_FMT_RESULTS); /* BRenderModern: NB: Detached later.*/
    ...
    for(br_size_t i = 0; i < data->nodes_count; ++i) {
        ...
        switch(a->type) {
            ...
            case BR_ACTOR_LIGHT:
                /* BRenderModern: FIXME: non-brender-lights? */
