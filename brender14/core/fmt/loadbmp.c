...
typedef struct {
    short bfType;
    int   bfSize; /* BRender: size of image file				*/
    short bfReserved1;
    short bfReserved2;
    int   bfOffBits; /* BRender: size of header	(including palette)	*/
} BITMAPFILEHEADER;

typedef struct {
    int   biSize;          /* BRender: size of BITMAPINFOHEADER always 0x28		*/
    int   biWidth;         /* BRender: width in pixels				*/
    int   biHeight;        /* BRender: depth in pixels				*/
    short biPlanes;        /* BRender: always 1					*/
    short biBitCount;      /* BRender: bits per pixel: 1,4,8,16,24,32		*/
    int   biCompression;   /* BRender: always 0, however spec exists for compression*/
    int   biSizeImage;     /* BRender: number of bytes in image			*/
    int   biXPelsPerMeter; /* BRender: resolution					*/
    int   biYPelsPerMeter; /* BRender: resolution					*/
    int   biClrUsed;       /* BRender: Number of colours users, 0=all ie. 8 bit=256	*/
    int   biClrImportant;  /* BRender: Number of important colours? 0=all		*/
} BITMAPINFOHEADER, *ptrBITMAPINFOHEADER;

typedef struct {
    int   bcSize; /* BRender: size of BITMAPINFOHEADER always 0x0c		*/
    short bcWidth;
    short bcHeight;
    short bcPlanes;
    short bcBitCount;
...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtBMPLoad(const char *name, br_uint_32 flags)
{
    ...
    switch(bi.biBitCount) {
        ...
        case(4): /* BRender: promote 4 to 8 bit */
...
int FileNumCol(void *tmp)
{
    ...
    switch(bits) {
        ...
        case(8):
            ...
            break;
        default:
            return 0; /* BRender: 24 bit has no palette */
    }
}
