...
static void *ReadLine_Uncomp_8(char *line, void *fh); /* BRender: 8 bit indexed, uncompressed	*/
static void *ReadLine_Comp_8(char *line, void *fh);   /* BRender: 8 bit indexed, compressed	*/

static void *ReadLine_Uncomp_16(char *line, void *fh); /* BRender: 16 bit, uncompressed		*/
static void *ReadLine_Comp_16(char *line, void *fh);   /* BRender: 16 bit, compressed		*/

static void *ReadLine_Uncomp_24(char *line, void *fh); /* BRender: 24 bit, uncompressed		*/
static void *ReadLine_Comp_24(char *line, void *fh);   /* BRender: 24 bit, compressed		*/

static void *ReadLine_Uncomp_32(char *line, void *fh); /* BRender: 32 bit, uncompressed		*/
static void *ReadLine_Comp_32(char *line, void *fh);   /* BRender: 32 bit, compressed		*/
...
enum {
    ...
    BR_TGA_UNCOMPRESSED_MONO    = 0x03, /* BRender: not supported here	*/
    BR_TGA_COMPRESSED_PALETTE   = 0x09,
    BR_TGA_COMPRESSED_RGB       = 0x0a,
    BR_TGA_COMPRESSED_MONO      = 0x0b /* BRender: not supported here	*/
};

enum {
    BR_TGA_REVERSED_WIDTH = 0x10, /* BRender: right to left 	*/
    BR_TGA_REVERSED_DEPTH = 0x20  /* BRender: bottom to top	*/
};
...
static struct {
    ...
    char pixel_bits;
    char supported;
    void *(*read_func)(char *line, void *fh);                       /* BRender: read 1 line of image data from file */
    void (*copy_func)(br_pixelmap *map, const char *line, int row); /* BRender: copy 1 line of data to pixelmap */
...
static struct {
    char           identsize;       /* BRender: number of bytes between header and whatever next 	*/
    char           colourmaptype;   /* BRender: type of colourmap in file 0=RGB, 1=palette		*/
    char           imagetype;       /* BRender: type of storage of image				*/
    unsigned short colourmapstart;  /* BRender: first colour index used in palette			*/
    unsigned short colourmaplength; /* BRender: number of colours in palette from colourmapstart	*/
    char           colourmapbits;   /* BRender: size of entry in palette table			*/
    unsigned short xstart;          /* BRender: offset between uppr lft of image & uppr lft of screen*/
    unsigned short ystart;
    unsigned short width;      /* BRender: width in pixels					*/
    unsigned short depth;      /* BRender: depth in pixels					*/
    char           bits;       /* BRender: number of bits of colour: 1,8,16,24,32		*/
    char           descriptor; /* BRender: bit 5 : set, image stored starting with last line	*/
                               /* BRender: bit 4 : set, image stored pixels right to left	*/
                               /* BRender: bits 0-3: number of bits available for overlays (n/a)*/
} TGAheader;
...
static void *ReadLine_Comp_8(char *line, void *fh)
{
    ...
    do {
        ...
        if(ch & 0x80) /* BRender: run of pixels */
        {
            if(BrFileRead(&ch, 1, 1, fh) != 1)
                return NULL;
