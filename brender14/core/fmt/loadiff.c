...
#define PACKET_SIZE     4 /* BRender: block size	*/

#define MAX_PALETTE     768 /* BRender: 256 colours	*/

#define RUN_LENGTH      0x80 /* BRender: rle bit field */
...
static struct {
    char         type[4];    /* BRender: iff/lbm sig, 'FORM','LIST' or 'CAT '			*/
    unsigned int size;       /* BRender: swapl(n) approx. size of file			*/
    char         subtype[4]; /* BRender: 'ILBM'  - image planes or 'PBM ' - byte oriented	*/
} IFFheader;

static struct {
    unsigned short w;                 /* BRender: width of image in pixels				*/
    unsigned short h;                 /* BRender: height of image in pixels				*/
    short          x;                 /* BRender: x coord of uppr left of image relative to screen	*/
    short          y;                 /* BRender: y coord or uppr left of image relative to screen	*/
    char           nPlanes;           /* BRender: number of colour planes required to display image (n/a)*/
    char           masking;           /* BRender: image data interleaved with mask			*/
                                      /* BRender: 0 - no mask, 1 - mask				*/
                                      /* BRender: 2 - mask with transparent colour, 3 - lasso mask	*/
    char           compression;       /* BRender: 0 - uncompressed image data, 1 - compressed (rle)	*/
    char           padl;              /* BRender: pad byte n/a						*/
    unsigned short transparentColour; /* BRender: colour index in image considered transparent		*/
    char           xAspect;           /* BRender: aspect ratio						*/
    char           yAspect;           /* BRender: aspect ratio						*/
    short          pageW;             /* BRender: size of source screen				*/
    short          pageH;             /* BRender: size of source screen				*/
} BMHD;
...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtIFFLoad(const char *name, br_uint_32 flags)
{
    ...
    if(!BrMemCmp(IFFheader.type, "FORM", 4) || !BrMemCmp(IFFheader.type, "LIST", 4) || !BrMemCmp(IFFheader.type, "CAT ", 4)) {
        do {
            ...
            if(block_length & 1)
                ++block_length; /* BRender: make block size even */
            ...
            if(!BrMemCmp(block, BITMAP_HEADER, 4)) {
                ...
                for(i = 0; i < BMHD.h; i++) {
                    ...
                    if(!BrMemCmp(IFFheader.subtype, "ILBM", 4) || (!BrMemCmp(IFFheader.subtype, "PBM ", 4) && BMHD.nPlanes < 8))
                        /* BRender: bitmap split into image planes */
                ...
                BrFileAdvance(block_length, fh); /* BRender: skip unknown block */
        } while(!BrFileEof(fh) && BrMemCmp(block, IMAGE_DATA, 4));

        pm->map = palette; /* BRender: palette may be after image block in file */
...
static int readline(char *mblock, int bytes, void *fh)
{
    ...
    do {
        ...
        if(c & RUN_LENGTH) /* BRender: run length	*/
        {
            if(c != 0x80) {
                i = ((~c) & 0xff) + 2;
                c = BrFileGetChar(fh);
...
static void planes2bytes(br_pixelmap *pm, char *block, int line)
{
    ...
    for(i = 0; i < BMHD.w; ++i) /* BRender: scan through pixels */
    {
        ((char *)(pm->pixels))[line * pm->row_bytes + i] = 0;
        src                                              = block;

        for(j = 0; j < BMHD.nPlanes; ++j) /* BRender: fetch each planar pixel */
        {
