...
// BRender: Pointer to actor plus desired transform matrix.
struct vue_transform {
    br_actor   *actor; // BRender: slot for actor to be moved by vue
    br_matrix34 mat;   // BRender: by this matrix
};

// BRender: All desired actor transforms required for a single frame.

struct vue_frame {
    struct vue_transform *transforms;  // BRender: actor/transforms
    int                   ntransforms; // BRender: number of transforms for that frame
};

typedef struct br_vue {
    int               nframes;
    int               ntransforms;
    int               frame_delta;   // BRender: default=1
    int               current_frame; // BRender: initialised to zero
