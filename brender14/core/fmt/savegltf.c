...
typedef struct br_gltf_save_state {
    cgltf_data *data;
    br_hashmap *actor_map;    /** BRenderModern: < br_actor                 -> cgltf_node     */
    br_hashmap *model_map;    /** BRenderModern: < br_model                 -> cgltf_mesh     */
    br_hashmap *material_map; /** BRenderModern: < br_material              -> cgltf_material */
    br_hashmap *stored_map;   /** BRenderModern: < br_material::stored      -> br_material    */
    br_hashmap *pixelmap_map; /** BRenderModern: < br_pixelmap              -> cgltf_image    */
    br_hashmap *sampler_map;  /** BRenderModern: < br_gltf_save_sampler_key -> cgltf_sampler  */
...
static void build_actor_lookup(br_hashmap *hm, br_actor *root) // BRenderModern: NOLINT(*-no-recursion)
{
...
static int gather_samplers(const void *key, void *value, br_hash hash, void *user)
{
    ...
    if(sampler == NULL) {
        ...
        *new_key                          = sampler_key;
        BrHashMapInsert(state->sampler_map, new_key, (void *)1); /* BRenderModern: FIXME: using a dummy, non-NULL value here. */
    }
...
static int count_model_buffers(const void *key, void *value, br_hash hash, void *user)
{
    ...
    br_size_t              nbuffers = BR_ASIZE(attribute_handlers) + 1; /* BRenderModern: attribs + index */
...
static int fill_material(const void *key, void *value, br_hash hash, void *user)
{
    ...
    /* BRenderModern: TODO: Figure out how to decompose map_transform. */
    // BRenderModern: material->pbr_metallic_roughness.base_color_texture.has_transform = true;
    ...
    if(mat->flags & (BR_MATF_ENVIRONMENT_I | BR_MATF_ENVIRONMENT_L)) {
        ...
        material->pbr_metallic_roughness.roughness_factor = 0.05f; /* BRenderModern: sharp reflection */
    } else if(mat->ks > 0.0f && mat->power > 0.0f) {
        /* BRenderModern: Blinn-Phong power to roughness approximation */
