...
static br_3ds_options _BrDefault3DSOptions = {
     ...
     * Flags
     */
    BR_3DS_CORRECT_AXES | BR_3DS_APPLY_MESH_MATRIX | BR_3DS_APPLY_PIVOT | BR_3DS_SET_HITHER | BR_3DS_SET_YON,

    /* BRender:
     * Hither and yon
     */
    BR_SCALAR(1.0), BR_SCALAR(10.0),
...
static Bool_t ReadChar(void *stream, char *result)
{
    ...
    if((c = BrFileGetChar(stream)) == BR_EOF)
        return FALSE;
...
static Bool_t ReadUInt8(void *stream, br_uint_8 *result)
{
    ...
    if((b = BrFileGetChar(stream)) == BR_EOF)
        return FALSE;
...
static Bool_t ReadUInt16(void *stream, br_uint_16 *result)
{
    ...
    if((lsb = BrFileGetChar(stream)) == BR_EOF)
        return FALSE;
    if((msb = BrFileGetChar(stream)) == BR_EOF)
        return FALSE;
...
static Bool_t ReadUInt32(void *stream, br_uint_32 *result)
{
    ...
    for(i = 0; i < 4; i++)
        if((b[i] = BrFileGetChar(stream)) == BR_EOF)
            return FALSE;
...
{
    return BR_COLOUR_RGB((float)(255.0 * color->red), (float)(255.0 * color->green), (float)(255.0 * color->blue));
}
...
static void UpdatePixmapRef(PixmapRef_t *exist_ref, PixmapRef_t *new_ref)
{
    ...
    if(exist_ref->mat_mapname == NULL || new_ref->strength > exist_ref->strength) {
...
static MaterialList_t *ConvertMaterial(MatEntry_t *mat_entry, PixmapList_t **pixmap_list, br_3ds_options *options)
{
    ...
    /* BRender: Set the RGB true colour. This will only be used when rendering
...
static State_t ReadFaceArray(void *stream, Stack_t *top)
{
    ...
    if(!ReadUInt16(stream, &n_faces) || n_faces == 0) {
        return PARSE_ERROR;
    }
...
static State_t ReadTexVerts(void *stream, Stack_t *top)
{
    ...
    if(!ReadUInt16(stream, &n_texverts) || n_texverts == 0) {
        return PARSE_ERROR;
    }
...
static State_t ConvertNTriObj(NTriObj_t *n_tri_obj, NamedObj_t *named_obj, br_3ds_options *options)
{
    ...
    if((n_fill_faces > 0 && model->fill_model == NULL) || (n_wire_faces > 0 && model->wire_model == NULL)) {
...
static State_t ConvertNCamera(NCamera_t *n_camera, NamedObj_t *named_obj, br_3ds_options *options)
{
    ...
    MakeMatrixFromTarget(&(named_obj->to_world), &(n_camera->posn), &(n_camera->target), n_camera->bank_angle);
    ...
    if(options->flags & BR_3DS_DUMMY_CAMERAS) {
        ...
        if(options->report != NULL) {
            ReportMessage(options->report, "Focal length: %5.1f mm Angle: %5.1f degrees\n", n_camera->focal_length,
                          BrScalarToFloat(BrAngleToDegree(fov)));
        }
#endif
...
static State_t ConvertNDLight(NDLight_t *n_d_light, NamedObj_t *named_obj, br_3ds_options *options)
{
    ...
    if(n_d_light->is_spotlight) {
        MakeMatrixFromTarget(&(named_obj->to_world), &(n_d_light->posn), &(n_d_light->dl_spotlight.target), 0.0F);
    } else {
        BrMatrix34Translate(&(named_obj->to_world), n_d_light->posn.v[0], n_d_light->posn.v[1], n_d_light->posn.v[2]);
    }
    ...
    if(options->flags & BR_3DS_DUMMY_LIGHTS) {
        ...
        if(n_d_light->is_spotlight) {
            ...
            light->cone_inner = BrDegreeToAngle(BrFloatToScalar((float)(n_d_light->dl_spotlight.cone_inner)));
            light->cone_outer = BrDegreeToAngle(BrFloatToScalar((float)(n_d_light->dl_spotlight.cone_inner)));
...
static br_uint_32 BR_CALLBACK DetachCameras(br_actor *actor, void *arg)
{
    ...
    if(actor->type == BR_ACTOR_CAMERA)
        BrResAdd(results->cameras, actor->type_data);
...
static br_uint_32 BR_CALLBACK DetachLights(br_actor *actor, void *arg)
{
    ...
    if(actor->type == BR_ACTOR_LIGHT)
        BrResAdd(results->lights, actor->type_data);
    ...
    for(item = list; item != NULL; item = item->next) {
        if(item->type == MODEL) {
            if(item->data.model->fill_model != NULL)
                ModelScale(item->data.model->fill_model, &vscale);
            if(item->data.model->wire_model != NULL)
                ModelScale(item->data.model->wire_model, &vscale);
        }
    }
}
...
static void CollectModels(br_fmt_results *results, NamedObj_t *list)
{
    ...
    for(i = 0, item = list; item != NULL; item = item->next) {
        if(item->type == MODEL) {
            if(item->data.model->fill_model != NULL)
                results->models[i++] = item->data.model->fill_model;
            if(item->data.model->wire_model != NULL)
                results->models[i++] = item->data.model->wire_model;
        }
    }
     ...
     */
    for(i = 0; i < count; i++)
        BrResAdd(results->models, results->models[i]);
}
...
static void CollectPixelmaps(br_fmt_results *results, PixmapList_t *list)
{
     ...
     */
    for(i = 0, item = list; item != NULL; item = item->next)
        results->pixelmaps[i++] = item->pixelmap;

    /* BRender:
     * Attach pixelmaps to table as resources
     */
    for(i = 0; i < count; i++)
        BrResAdd(results->pixelmaps, results->pixelmaps[i]);
}
...
static void CollectMaterials(br_fmt_results *results, MaterialList_t *list)
{
     ...
     */
    for(i = 0, item = list; item != NULL; item = item->next)
        results->materials[i++] = item->material;

    /* BRender:
     * Attach materials to table as resources
     */
    for(i = 0; i < count; i++)
        BrResAdd(results->materials, results->materials[i]);
}
...
static State_t ReadNDLight(void *stream, Stack_t *top, br_3ds_options *options)
{
    ...
    if(options->report != NULL) {
        ...
        ReportMessage(options->report, "  position: (%f %f %f)\n", BrScalarToFloat(n_d_light->posn.v[0]),
                      BrScalarToFloat(n_d_light->posn.v[1]), BrScalarToFloat(n_d_light->posn.v[2]));
    }
#endif
...
static State_t ReadNCamera(void *stream, Stack_t *top, br_3ds_options *options)
{
    ...
    if(!ReadPoint(stream, top, &(n_camera->posn), options) || !ReadPoint(stream, top, &(n_camera->target), options) ||
       !ReadFloat(stream, &(n_camera->bank_angle)) || !ReadFloat(stream, &(n_camera->focal_length))) {
        return PARSE_ERROR;
    }
    ...
    if(options->report != NULL) {
        ...
        ReportMessage(options->report, "  position: (%f %f %f)\n", BrScalarToFloat(n_camera->posn.v[0]),
                      BrScalarToFloat(n_camera->posn.v[1]), BrScalarToFloat(n_camera->posn.v[2]));
        ReportMessage(options->report, "  target: (%f %f %f)\n", BrScalarToFloat(n_camera->target.v[0]),
                      BrScalarToFloat(n_camera->target.v[1]), BrScalarToFloat(n_camera->target.v[2]));
    }
#endif
...
static Bool_t InsertNodeTag(NodeTag_t *new_tag, NodeTag_t **tags)
{
    ...
    if(named_obj == NULL) {
        ...
        for(tag = *tags; tag != NULL; tag = tag->next) {
            if(tag->node_hdr.named_obj == NULL && SameString(tag->instance_name, new_tag->instance_name)) {
                return FALSE;
            }
            link = &(tag->next);
        }
        ...
        for(tag = *tags; tag != NULL; tag = tag->next) {
            if(tag->node_hdr.named_obj != NULL && SameString(named_obj->name, tag->node_hdr.named_obj->name) &&
 ...
 */
static br_actor *MakeEnclosingActor(Int_t depth, char *instance_name, NamedObj_t *named_obj, br_matrix34 *world_to_parent,
                                    br_matrix34 *world_to_actor_ret, br_vector3 *pivot, br_3ds_options *options)
{
    char      *actor_name;
    br_actor  *actor;
    br_actor  *fill_actor;
    ...
    if(named_obj != NULL) {
        ...
        if(pivot) {
            BrMatrix34PreTranslate(&(named_obj->to_world), pivot->v[0], pivot->v[1], pivot->v[2]);
            BrMatrix34PostTranslate(&(named_obj->from_world), -pivot->v[0], -pivot->v[1], -pivot->v[2]);
        }
    ...
    if(named_obj != NULL && named_obj->type == LIGHT) {
        ...
        if(options->flags & BR_3DS_SET_HITHER)
            camera->hither_z = options->hither;
        if(options->flags & BR_3DS_SET_YON)
            camera->yon_z = options->yon;

    } else if(named_obj != NULL && named_obj->type == MODEL && named_obj->data.model->fill_model != NULL &&
        ...
        if(pivot) {
            if(options->flags & BR_3DS_USE_MODEL_PIVOT)
                ...
                TranslateModelVertices(fill_actor->model, -pivot->v[0], -pivot->v[1], -pivot->v[2]);
        }
        ...
        if(pivot) {
            if(options->flags & BR_3DS_USE_MODEL_PIVOT)
                ...
                TranslateModelVertices(wire_actor->model, -pivot->v[0], -pivot->v[1], -pivot->v[2]);
        }
        ...
        if(pivot) {
            if(options->flags & BR_3DS_USE_MODEL_PIVOT)
                ...
                TranslateModelVertices(actor->model, -pivot->v[0], -pivot->v[1], -pivot->v[2]);
        }
    } else if(named_obj != NULL && named_obj->type == MODEL && named_obj->data.model->fill_model == NULL &&
              named_obj->data.model->wire_model != NULL) {
                                 ...
                                 br_3ds_options *options)
{
    ...
    for(node_tag = node_tags; node_tag != NULL; node_tag = node_tag->next) {
        ...
        if(node_tag->node_hdr.parent == parent) {
             ...
             */
            if(!CollectDescendents(depth + 1, child, node_tag->node_hdr.index, node_tags, &world_to_child, options) ||
...
br_fmt_results *BR_PUBLIC_ENTRY BrFmt3DSLoad(const char *name, br_fmt_options *fmt_options, br_3ds_options *options)
{
    ...
    while((top >= stack) && (state == OK)) {
        ...
        if((!ReadHeader(stream, top)) || (top->length < 6) || (top->length > (parent->length - parent->done))) {
            ...
            switch(top->id_tag) {
                ...
                case INT_PERCENTAGE:
                    if(((parent->id_tag == MAT_SHININESS) || (parent->id_tag == MAT_TRANSPARENCY) || (parent->id_tag == MAT_TEXMAP) ||
                       ...
                       (parent->flags & GOT_INT_PERCENTAGE) == 0 && (parent->flags & GOT_FLOAT_PERCENTAGE) == 0) {
                        ...
                        if(ReadInt16(stream, &percent) && percent >= 0 && percent <= 100) {
                            /* BRender:
                ...
                case M3D_VERSION:
                    if(parent->id_tag == M3DMAGIC && (parent->flags & GOT_M3D_VERSION) == 0) {
                        br_int_32 version;
                ...
                case MESH_VERSION:
                    if(parent->id_tag == MDATA && (parent->flags & GOT_MESH_VERSION) == 0) {
                        br_int_32 version;
                ...
                case MASTER_SCALE:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_MASTER_SCALE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case LO_SHADOW_BIAS:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_LO_SHADOW_BIAS) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case HI_SHADOW_BIAS:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_HI_SHADOW_BIAS) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case SHADOW_MAP_SIZE:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_SHADOW_MAP_SIZE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case SHADOW_SAMPLES:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_SHADOW_SAMPLES) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case SHADOW_RANGE:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_SHADOW_RANGE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case SHADOW_FILTER:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_SHADOW_FILTER) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case AMBIENT_LIGHT:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_AMBIENT_LIGHT) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case O_CONSTS:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_O_CONSTS) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case BIT_MAP:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_BIT_MAP) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case SOLID_BGND:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_SOLID_BGND) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case V_GRADIENT:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_V_GRADIENT) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case USE_BIT_MAP:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_USE_BIT_MAP) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case USE_SOLID_BGND:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_USE_SOLID_BGND) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case USE_V_GRADIENT:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_USE_V_GRADIENT) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case FOG:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_FOG) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case DISTANCE_CUE:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_DISTANCE_CUE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case USE_FOG:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_USE_FOG) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case USE_DISTANCE_CUE:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_USE_DISTANCE_CUE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case DEFAULT_VIEW:
                    if(parent->id_tag != MDATA || (parent->flags & GOT_DEFAULT_VIEW) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case VIEW_TOP: /* BRender:  fall through .....  */
                case VIEW_BOTTOM:
                case VIEW_LEFT:
                case VIEW_RIGHT:
                case VIEW_FRONT:
                case VIEW_BACK:
                case VIEW_USER:
                case VIEW_CAMERA: /* BRender:  ..... until here  */
                    state = PARSE_ERROR;
                    break;
                ...
                case MAT_NAME:
                    top->data.string = NULL;
                    if(parent->id_tag == MAT_ENTRY && (parent->flags & GOT_MAT_NAME) == 0) {
                        char buffer[17];

                        if(ReadString(stream, top, 17, buffer) && LookupMaterial(buffer, material_list) == NULL) {
                            top->data.string = BrMemStrDup(buffer);
                            if(top->data.string == NULL) {
                                state = OUT_OF_MEMORY;
                            }
                ...
                case MAT_AMBIENT:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_AMBIENT) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_DIFFUSE:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_DIFFUSE) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_SPECULAR:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_SPECULAR) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_SHININESS:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_SHININESS) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_TRANSPARENCY:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_TRANSPARENCY) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_XPFALL:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_XPFALL) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_USE_XPFALL:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_USE_XPFALL) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_REFBLUR:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_REFBLUR) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_USE_REFBLUR:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_USE_REFBLUR) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_SELF_ILLUM:
                    if(parent->id_tag != MAT_ENTRY || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_TWO_SIDE:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_TWO_SIDE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_DECAL:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_DECAL) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case MAT_SHADING:
                    if(parent->id_tag == MAT_ENTRY && (parent->flags & GOT_MAT_SHADING) == 0) {
                        br_int_16 shading;
                        ...
                        if(ReadInt16(stream, &shading)) {
                            ...
                            switch(shading) {
                                case 0:
                                    top->data.mat_shading = WIREFRAME;
                                    break;
                                case 1:
                                    top->data.mat_shading = FLAT;
                                    break;
                                default:
                                    top->data.mat_shading = GOURAUD;
                                    break;
                            }
                        } else {
                            state = PARSE_ERROR;
                ...
                case MAT_TEXMAP:
                    ...
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_TEXMAP) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case MAT_OPACMAP:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_OPACMAP) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case MAT_REFLMAP:
                    InitialisePixmapRef(&(top->data.pixmap_ref));
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_REFLMAP) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_ACUBIC:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_ACUBIC) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case MAT_BUMPMAP:
                    if(parent->id_tag != MAT_ENTRY || (parent->flags & GOT_MAT_BUMPMAP) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case NAMED_OBJECT:
                    ...
                    if(parent->id_tag == MDATA) {
                        ...
                        if(ReadString(stream, top, 12, buffer) && LookupNamedObj(buffer, named_objs) == NULL) {
                ...
                case N_TRI_OBJECT:
                    ...
                    if(parent->id_tag == NAMED_OBJECT && (parent->flags & GOT_N_TRI_OBJECT) == 0 &&
                       (parent->flags & GOT_N_DIRECT_LIGHT) == 0 && (parent->flags & GOT_N_CAMERA) == 0) {
                ...
                case POINT_ARRAY:
                    ...
                    if(parent->id_tag == N_TRI_OBJECT && (parent->flags & GOT_POINT_ARRAY) == 0) {
                ...
                case FACE_ARRAY:
                    ...
                    if(parent->id_tag == N_TRI_OBJECT && (parent->flags & GOT_FACE_ARRAY) == 0) {
                ...
                case SMOOTH_GROUP:
                    ...
                    if(parent->id_tag == FACE_ARRAY && (parent->flags & GOT_SMOOTH_GROUP) == 0) {
                        state = ReadSmoothGroup(stream, top, parent->data.face_array.n_faces, options);
                    } else {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case TEX_VERTS:
                    ...
                    if(parent->id_tag == N_TRI_OBJECT && (parent->flags & GOT_TEX_VERTS) == 0) {
                ...
                case MESH_MATRIX:
                    if(parent->id_tag == N_TRI_OBJECT && (parent->flags & GOT_MESH_MATRIX) == 0) {
                ...
                case PROC_NAME:
                    if(parent->id_tag != N_TRI_OBJECT || (parent->flags & GOT_PROC_NAME) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case PROC_DATA:
                    if(parent->id_tag != N_TRI_OBJECT || (parent->flags & GOT_PROC_DATA) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case DL_OFF:
                    if(parent->id_tag == N_DIRECT_LIGHT && (parent->flags & GOT_DL_OFF) == 0 && SkipRest(stream, top)) {
                ...
                case DL_SPOTLIGHT:
                    if(parent->id_tag == N_DIRECT_LIGHT && (parent->flags & GOT_DL_SPOTLIGHT) == 0) {
                ...
                case DL_SHADOWED: /* BRender:  fall through .....  */
                case DL_LOCAL_SHADOW2:
                case DL_SEE_CONE: /* BRender:  ..... until here    */
                    state = PARSE_ERROR;
                    break;
                ...
                case OBJ_HIDDEN:
                    if(parent->id_tag != NAMED_OBJECT || (parent->flags & GOT_OBJ_HIDDEN) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case OBJ_MATTE:
                    if(parent->id_tag != NAMED_OBJECT || (parent->flags & GOT_OBJ_MATTE) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case OBJ_FAST:
                    if(parent->id_tag != NAMED_OBJECT || (parent->flags & GOT_OBJ_FAST) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case OBJ_FROZEN:
                    if(parent->id_tag != NAMED_OBJECT || (parent->flags & GOT_OBJ_FROZEN) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case KFDATA:
                    if(parent->id_tag != M3DMAGIC || (parent->flags & GOT_KFDATA) != 0) {
                        state = PARSE_ERROR;
                    }
                    break;

                case KFHDR:
                    if(parent->id_tag == KFDATA && (parent->flags & GOT_KFHDR) == 0) {
                        br_int_16 version;
                        ...
                        if(ReadInt16(stream, &version)) {
                            ...
                            if(options->report != NULL) {
                                if((version == 1) || (version == 2)) {
                                    ...
                                    ReportMessage(options->report,
                                                  "Keyframe data version %d.0 (only able to read version 1.0 and 2.0 "
                                                  "chunks)\n",
                                                  version);
                                }
                            }
                ...
                case KFSEG:
                    if(parent->id_tag != KFDATA || (parent->flags & GOT_KFSEG) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;

                case KFCURTIME:
                    if(parent->id_tag != KFDATA || (parent->flags & GOT_KFCURTIME) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case TARGET_NODE_TAG:
                    top->data.node_tag = NULL;
                    if(parent->id_tag != KFDATA || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case PIVOT:
                    if(parent->id_tag == OBJECT_NODE_TAG && (parent->flags & GOT_PIVOT) == 0) {
                ...
                case INSTANCE_NAME:
                    top->data.string = NULL;
                    if(parent->id_tag == OBJECT_NODE_TAG && (parent->flags & GOT_INSTANCE_NAME) == 0) {
                        char buffer[12];
                ...
                case BOUNDBOX:
                    if(parent->id_tag == OBJECT_NODE_TAG && (parent->flags & GOT_BOUNDBOX) == 0) {
                        br_vector3 *min;
                        br_vector3 *max;
                        ...
                        if(ReadPoint(stream, top, min, options) && ReadPoint(stream, top, max, options)) {
                ...
                case COL_TRACK_TAG:
                    if((parent->id_tag != SPOTLIGHT_NODE_TAG && parent->id_tag != LIGHT_NODE_TAG) ||
                       (parent->flags & GOT_COL_TRACK_TAG) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
                ...
                case ROLL_TRACK_TAG:
                    if((parent->id_tag != CAMERA_NODE_TAG && parent->id_tag != SPOTLIGHT_NODE_TAG) ||
                       (parent->flags & GOT_ROLL_TRACK_TAG) != 0 || !SkipRest(stream, top)) {
                        state = PARSE_ERROR;
                    }
                    break;
        ...
        while((top >= stack) && ((state != OK) || (top->done == top->length))) {
            ...
            switch(top->id_tag) {
                ...
                case COLOR_F:
                    if(state == OK) {
                        if(parent->id_tag == N_DIRECT_LIGHT) {
                            BlockCopy(&(parent->data.n_d_light.color), &(top->data.color), sizeof(Color_t));
                        } else {
                            BlockCopy(&(parent->data.color), &(top->data.color), sizeof(Color_t));
                        }
                ...
                case INT_PERCENTAGE:
                    if(state == OK) {
                        if(parent->id_tag == MAT_TEXMAP || parent->id_tag == MAT_REFLMAP) {
                            parent->data.pixmap_ref.strength = top->data.percent;
                        } else {
                ...
                case MAT_ENTRY:
                    if(state == OK) {
                        if(top->flags & GOT_MAT_NAME) {
                            ...
                            material_link = ConvertMaterial(&(top->data.mat_entry), &pixmap_list, options);
                ...
                case MAT_AMBIENT:
                    if(state == OK) {
                        if((top->flags & GOT_COLOR_F) != 0 || (top->flags & GOT_COLOR_24) != 0) {
                            BlockCopy(&(parent->data.mat_entry.mat_ambient), &(top->data.color), sizeof(Color_t));
                ...
                case MAT_DIFFUSE:
                    if(state == OK) {
                        if((top->flags & GOT_COLOR_F) != 0 || (top->flags & GOT_COLOR_24) != 0) {
                            BlockCopy(&(parent->data.mat_entry.mat_diffuse), &(top->data.color), sizeof(Color_t));
                ...
                case MAT_SPECULAR:
                    if(state == OK) {
                        if((top->flags & GOT_COLOR_F) != 0 || (top->flags & GOT_COLOR_24) != 0) {
                            BlockCopy(&(parent->data.mat_entry.mat_specular), &(top->data.color), sizeof(Color_t));
                ...
                case MAT_SHININESS:
                    if(state == OK) {
                        if((top->flags & GOT_FLOAT_PERCENTAGE) != 0 || (top->flags & GOT_INT_PERCENTAGE) != 0) {
                            parent->data.mat_entry.mat_shininess = top->data.percent;
                ...
                case MAT_TRANSPARENCY:
                    if(state == OK) {
                        if((top->flags & GOT_FLOAT_PERCENTAGE) != 0 || (top->flags & GOT_INT_PERCENTAGE) != 0) {
                            parent->data.mat_entry.mat_transparency = top->data.percent;
                ...
                case MAT_SHADING:
                    if(state == OK) {
                        parent->data.mat_entry.mat_shading = top->data.mat_shading;
                ...
                case MAT_TEXMAP:
                    if(state == OK) {
                        if((top->flags & GOT_MAT_MAPNAME) != 0 &&
                           ((top->flags & GOT_FLOAT_PERCENTAGE) != 0 || (top->flags & GOT_INT_PERCENTAGE) != 0)) {
                            UpdatePixmapRef(&(parent->data.mat_entry.pixmap_ref), &(top->data.pixmap_ref));
                ...
                case N_TRI_OBJECT:
                    if(state == OK) {
                        if((top->flags & GOT_POINT_ARRAY) != 0 && (top->flags & GOT_FACE_ARRAY) != 0) {
                            state = ConvertNTriObj(&(top->data.n_tri_obj), parent->data.named_obj, options);
                            if(state == OK) {
                ...
                case POINT_ARRAY:
                    if(state == OK) {
                        ...
                        BlockCopy(&(parent->data.n_tri_obj.point_array), &(top->data.point_array), sizeof(PointArray_t));
                ...
                case FACE_ARRAY:
                    if(state == OK) {
                        ...
                        BlockCopy(&(parent->data.n_tri_obj.face_array), &(top->data.face_array), sizeof(FaceArray_t));
                ...
                case MSH_MAT_GROUP:
                    if(state == OK) {
                        top->data.msh_mat_group->next          = parent->data.face_array.msh_mat_groups;
                        parent->data.face_array.msh_mat_groups = top->data.msh_mat_group;
                    } else {
                ...
                case SMOOTH_GROUP:
                    if(state == OK) {
                        parent->data.face_array.smooth_group = top->data.smooth_group;
                ...
                case TEX_VERTS:
                    if(state == OK) {
                        ...
                        BlockCopy(&(parent->data.n_tri_obj.tex_verts), &(top->data.tex_verts), sizeof(TexVerts_t));
                ...
                case MESH_MATRIX:
                    if(state == OK) {
                        BrMatrix34Copy(&(parent->data.n_tri_obj.mesh_matrix), &(top->data.mesh_matrix));
                ...
                case N_DIRECT_LIGHT:
                    if(state == OK) {
                        if((top->flags & GOT_COLOR_F) != 0 || (top->flags & GOT_COLOR_24) != 0) {
                            ConvertNDLight(&(top->data.n_d_light), parent->data.named_obj, options);
                ...
                case DL_SPOTLIGHT:
                    if(state == OK) {
                        ...
                        BlockCopy(&(parent->data.n_d_light.dl_spotlight), &(top->data.dl_spotlight), sizeof(DlSpotlight_t));
                ...
                case N_CAMERA:
                    if(state == OK) {
                        ConvertNCamera(&(top->data.n_camera), parent->data.named_obj, options);
                ...
                case LIGHT_NODE_TAG:
                    if(state == OK) {
                        if((top->flags & GOT_NODE_HDR) != 0 && (top->flags & GOT_POS_TRACK_TAG) != 0 &&
                           (top->flags & GOT_COL_TRACK_TAG) != 0 && InsertNodeTag(top->data.node_tag, &node_tags)) {
                            n_node_tags += 1;
                        } else {
                            state = PARSE_ERROR;
                        }
                ...
                case SPOTLIGHT_NODE_TAG:
                    if(state == OK) {
                        if((top->flags & GOT_NODE_HDR) != 0 && (top->flags & GOT_POS_TRACK_TAG) != 0 &&
                           (top->flags & GOT_COL_TRACK_TAG) != 0 && (top->flags & GOT_HOT_TRACK_TAG) != 0 &&
                           (top->flags & GOT_FALL_TRACK_TAG) != 0 && InsertNodeTag(top->data.node_tag, &node_tags)) {
                            n_node_tags += 1;
                        } else {
                            state = PARSE_ERROR;
                        }
                ...
                case NODE_HDR:
                    if(state == OK) {
                        BlockCopy(&(parent->data.node_tag->node_hdr), &(top->data.node_hdr), sizeof(NodeHdr_t));
                ...
                case PIVOT:
                    if(state == OK) {
                        ...
                        BrVector3Copy(&(parent->data.node_tag->pivot), &(top->data.pivot));
                ...
                case INSTANCE_NAME:
                    if(state == OK) {
                        parent->data.node_tag->instance_name = top->data.string;
                ...
                case BOUNDBOX:
                    if(state == OK) {
                        ...
                        BrVector3Copy(&(parent->data.node_tag->boundbox.min), &(top->data.boundbox.min));
                        BrVector3Copy(&(parent->data.node_tag->boundbox.max), &(top->data.boundbox.max));
    ...
    if(fmt_options->convert_flags & (BR_FMT_ACTORS | BR_FMT_CAMERAS | BR_FMT_LIGHTS)) {
         ...
         * Collect cameras
         */
        if(fmt_options->convert_flags & BR_FMT_CAMERAS)
            CollectCameras(results, actor);

        /* BRender:
         * Collect lights
         */
        if(fmt_options->convert_flags & BR_FMT_LIGHTS)
            CollectLights(results, actor);
        ...
        if(fmt_options->convert_flags & BR_FMT_ACTORS) {
             ...
             */
            if(options->flags & BR_3DS_APPLY_SCALE)
                ScaleHierarchy(actor, options->scale);

            /* BRender:
             * Clear references to models and materials if not converted
             */
            if(!(fmt_options->convert_flags & BR_FMT_MODELS))
                BrActorEnum(actor, ClearActorModels, NULL);
            if(!(fmt_options->convert_flags & BR_FMT_MATERIALS))
                BrActorEnum(actor, ClearActorMaterials, NULL);
             ...
             */
            if(fmt_options->convert_flags & BR_FMT_CAMERAS)
                BrActorEnum(actor, DetachCameras, results);
            if(fmt_options->convert_flags & BR_FMT_LIGHTS)
                BrActorEnum(actor, DetachLights, results);

            /* BRender:
             * Free actor hierarchy
             */
            if(fmt_options->convert_flags & (BR_FMT_CAMERAS | BR_FMT_LIGHTS))
                BrActorFree(actor);
        }
    }
    ...
    if(fmt_options->convert_flags & BR_FMT_MODELS) {
         ...
         */
        if(options->flags & BR_3DS_APPLY_SCALE)
            ScaleModels(named_objs, options->scale);
    ...
    if(fmt_options->convert_flags & BR_FMT_MATERIALS) {
        ...
        if(!(fmt_options->convert_flags & BR_FMT_PIXELMAPS))
            ClearMaterialPixelmaps(results);
