...
static br_error GltfviewInit(br_demo *demo)
{
    ...
    /* BRenderModern: Derive base_path from model filename */
    {
    ...
    /* BRenderModern: Add all loaded actors to the world */
    ...
    /* BRenderModern: Register loaded resources (BrModelAdd calls BrModelUpdate internally) */
    ...
    /* BRenderModern: Compute scene bounds from model->bounds (populated by BrModelUpdate) */
    {
        bbox_ctx bb = {{{0}}, {{0}}, 0};
    ...
    /* BRenderModern: Apply brightness to all loaded materials */
    for(i = 0; i < (int)results->nmaterials; i++) {
    ...
    /* BRenderModern: Use animation data loaded by BrFmtGLTFActorLoadMany (if present) */
    ...
    /* BRenderModern: Apply center override */
...
static void GltfviewUpdate(br_demo *demo, br_scalar dt)
{
    ...
    /* BRenderModern: Evaluate animation */
    if(state->anim)
