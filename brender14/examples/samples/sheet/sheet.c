/* BRenderModern: A simple real time physical simulation */
...
/* BRenderModern: Specify number of vertices in sheet */
...
void InitialiseSheetModel(br_model *model)
{
    ...
    /* BRenderModern: Position vertices of rubber sheet */
    /* BRenderModern: The reverse side of each face uses a different texture */
...
/* BRenderModern: The centres of the three sphere */
...
/* BRenderModern: Calculate force due to fields centred on spheres. Force is only */
/* BRenderModern: applied to points within a threshold radius */
/* BRenderModern: Return 1 if point is within threshold */
...
int AccumulateSphereField(br_vector3 *a, br_vector3 *pos, br_vector3 *centre)
{
    ...
    if(length < RADIUS2) {
        ...
        /* BRenderModern: Force is proportional to square of distance over threshold */
...
int AccumulatePlaneField(br_vector3 *a, br_vector3 *pos, br_vector3 *normal, br_scalar k)
{
    ...
    if(length > 0) {
        ...
        /* BRenderModern: Force is proportional to distance over threshold */
...
/* BRenderModern: The sheet is made up of springs. CalculateSpring calculates the */
/* BRenderModern: extension of each spring and the resultant force if it is stretched */
/* BRenderModern: longer than its natural length */
...
void SheetUpdate(br_demo *demo, br_scalar dt)
{
    ...
    while(sheet->accum >= SPF) {
        ...
        /* BRenderModern: If the sheet has changed we need to tell BRender to */
        /* BRenderModern: update the normals, bounding box and radius */
