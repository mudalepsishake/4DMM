...
const static br_smpte_colours smpte_r8g8b8a8 = {
    .grey40    = BR_COLOUR_R8G8B8A8(104, 104, 104, 255),
    .white75   = BR_COLOUR_R8G8B8A8(180, 180, 180, 255),
    .yellow75  = BR_COLOUR_R8G8B8A8(180, 180, 16, 255),
    .cyan75    = BR_COLOUR_R8G8B8A8(16, 180, 180, 255),
    .green75   = BR_COLOUR_R8G8B8A8(16, 180, 16, 255),
    .magenta75 = BR_COLOUR_R8G8B8A8(180, 16, 180, 255),
    .red75     = BR_COLOUR_R8G8B8A8(180, 16, 16, 255),
    .blue75    = BR_COLOUR_R8G8B8A8(16, 16, 180, 255),
    .black75   = BR_COLOUR_R8G8B8A8(16, 16, 16, 255),
    .white100  = BR_COLOUR_R8G8B8A8(235, 235, 235, 255),
    .plusQ     = BR_COLOUR_R8G8B8A8(72, 16, 118, 255),
    .plusI     = BR_COLOUR_R8G8B8A8(106, 52, 16, 255),
    .minusI    = BR_COLOUR_R8G8B8A8(16, 70, 106, 255),
    .plugeL    = BR_COLOUR_R8G8B8A8(9, 9, 9, 255),
    .plugeR    = BR_COLOUR_R8G8B8A8(29, 29, 29, 255),
};
...
static void fill_smpte(br_pixelmap *pm, const br_smpte_colours *colours)
{
    const br_colour row1_colours[SMPTE_NUM_BARS] = {
        colours->white75, colours->yellow75, colours->cyan75, colours->green75, colours->magenta75, colours->red75, colours->blue75,
    };
    const br_colour row2_colours[SMPTE_NUM_BARS] = {
        colours->blue75, colours->black75, colours->magenta75, colours->black75, colours->cyan75, colours->black75, colours->grey40,
    };
    ...
    const br_colour pluge_colours[SMPTE_NUM_PLUGE_BARS] = {
        /* BRenderModern: leftmost one with intensity 4% below black (super-black or blacker than black, 3.5 IRE) */
        colours->plugeL,
        /* BRenderModern: a middle one with intensity exactly equal to black (7.5 IRE) */
        colours->black75,
        /* BRenderModern: a rightmost one with intensity 4% above black level (11.5 IRE) */
        colours->plugeR,
    };
    ...
    for(int i = 0; i < BR_ASIZE(regions); ++i) {
        BrPixelmapRectangleFill(pm, regions[i].rect.x, regions[i].rect.y, regions[i].rect.w, regions[i].rect.h, regions[i].colour);
    }
}
...
const static struct {
    ...
    // BRenderModern: {.type = BR_PMT_INDEX_4,   .colours = &smpte_index_4  }, /* BrPixelmapRectangleFill() does not work with this. */
    ...
    {.type = BR_PMT_RGB_888,   .colours = &smpte_rgba_8888}, /* BRenderModern: Ignores the alpha part. */
    {.type = BR_PMT_RGBA_8888, .colours = &smpte_rgba_8888},
    {.type = BR_PMT_RGBX_888,  .colours = &smpte_rgba_8888}, /* BRenderModern: Ignores the alpha part. */
    {.type = BR_PMT_BGR_555,   .colours = &smpte_bgr_555  },
    {.type = BR_PMT_ARGB_4444, .colours = &smpte_argb_4444},
    {.type = BR_PMT_R8G8B8A8,  .colours = &smpte_r8g8b8a8 },
};
