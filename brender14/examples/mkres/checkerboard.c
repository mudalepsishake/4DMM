...
br_pixelmap *mkres_make_checkerboard8_pixelmap(const char *name)
{
    ...
    /* BRenderModern: Build the palette */
    ...
    /* BRenderModern: NB: anything at index 0 is transparent. */
    col          = pal->pixels;
    col[BLACK]   = BR_COLOUR_RGB(0x00, 0x00, 0x00);
    col[MAGENTA] = BR_COLOUR_RGB(0xFF, 0x00, 0xFF);

    /* BRenderModern: Build the map. */
...
br_pixelmap *mkres_make_checkerboard32_pixelmap(const char *name)
{
    ...
    if((pm = BrPixelmapAllocate(BR_PMT_RGBX_888, 64, 64, NULL, 0)) == NULL)
        return NULL;

    pm->identifier = BrResStrDup(pm, name);

    make_board(pm, BR_COLOUR_RGBA(0, 0, 0, 0xFF), BR_COLOUR_RGBA(0xFF, 0x00, 0xFF, 0xFF));
