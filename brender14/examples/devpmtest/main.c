 ...
 */
#include <brender.h>
#include <brddi.h> /* BRenderModern: To poke the device's CLUT. */
#include <brglrend.h>
...
static void linepoint_draw(br_pixelmap *dest, float dt, void *user)
{
    ...
    // BRenderModern: draw_linepoint_test(dest, -dest->origin_x + 20, -200, dt, &state->h, &state->s, &state->l);
}
...
static void device2mem_swap(br_pixelmap *screen, br_pixelmap *backbuffer, void *user)
{
    ...
    /* BRenderModern: Capture the last frame, using a device->memory non-stretch copy. */
    BrPixelmapCopy(state->last_frame_memory, backbuffer);

    /* BRenderModern: Capture the last frame, using a device->device non-stretch copy. */
...
void submap_draw(br_pixelmap *dest, float dt, void *user)
{
    ...
    // BRenderModern: BrPixelmapFill(state->left_square2, 0xFFFFFFFF);
}
...
static void pixelquery_draw(br_pixelmap *dest, float dt, void *user)
{
    ...
    /* BRenderModern: Test device::getPixel(), device::setPixel() */
    ...
    /* BRenderModern: Test memory::getPixel(), device::setPixel() */
...
static void xbpp_draw(br_pixelmap *dest, float dt, void *user)
{
    ...
    // BRenderModern: BrPixelmapRectangleCopy(dest, -dest->origin_x, -dest->origin_y, state->dst, -src->origin_x, -src->origin_y, src->width, src->height);
...
int main(int argc, char **argv)
{
    ...
    int          want_screenshot;
    int          test_index = 0; /* BRenderModern: BR_ASIZE(tests) - 1; */
    ...
    r = BrDevBeginVar(&screen, DEVICE_NAME,
                      ...
                      BRT_HIDPI_B,        BR_TRUE,
                      /* BRenderModern: Set these if you dare... */
...
#if 0 /* BRenderModern: Set 1 to use a memory pixelmap as the backbuffer. */
    ...
    for(SDL_Event evt;;) {
        ...
        /* BRenderModern: Clear the screen black. */
        BrPixelmapFill(colour_buffer, BR_COLOUR_RGBA(0, 0, 0, 0xFF));

        /* BRenderModern: Run the test */
        {
        ...
        /* BRenderModern: Add in some text. */
        {
