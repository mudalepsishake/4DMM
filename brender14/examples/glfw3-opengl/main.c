...
static br_error glfw_gl_create_context(br_pixelmap *pm, br_device_gl_context_info *info, void *user)
{
    ...
    /* BRenderModern: GLFW doesn't support manual context creation, just return the window. */
...
static br_error glfw_gl_make_current(br_pixelmap *pm, void *ctx, void *user)
{
    ...
    // BRenderModern: ctx == state->window
...
static br_pixelmap *create_screen(GLFWwindow *window, glfw_gl_state *state)
{
    ...
    /* BRenderModern: No need to keep this around, glrend copies it. */
    br_device_gl_ext_procs gl_ext_procs = {
        ...
        .preswap_hook        = NULL, /* BRenderModern: This is where you'd render things like ImGui. */
        .free                = NULL, /* BRenderModern: Not needed for GLFW. */
        .handle_window_event = NULL, /* BRenderModern: GLFW doesn't have window events. */
        .user                = state,
    };
