...
static void FocusCamOnSceneCamera(br_editor_camera *ecam, br_actor *world)
{
    ...
    // BRenderModern: ecam->camera_data.hither_z      = scdata->hither_z;
    // BRenderModern: ecam->camera_data.yon_z         = scdata->yon_z;
}
    ...
    for(size_t i = 0; i < nmaps; ++i) {
        ...
        for(int y = 0; y < pm->height; y++) {
            for(int x = 0; x < pm->width; x++) {
                /* BRenderModern: Assumes we're BR_PMT_RGBA_8888. */
...
static br_error GLTFInit(br_demo *demo)
{
    ...
    if(demo->hw_accel) {
        ...
        // BRenderModern: opts.pm_type = BR_PMT_INDEX_8;
    } else {
...
static void GLTFRender(br_demo *demo)
{
    ...
    // BRenderModern: BrZsSceneRender(demo->world, demo->camera, demo->colour_buffer);
