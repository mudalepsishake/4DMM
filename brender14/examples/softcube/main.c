...
static void draw_info(br_pixelmap *screen, br_material *mat, br_colour text_colour)
{
    ...
    // BRenderModern: ObjectQuery(BrV1dbRendererQuery(), &primstate, pstate_token);
...
int main(int argc, char **argv)
{
    ...
    // BRenderModern: cube->material->flags |= BR_MATF_PERSPECTIVE; // Perspective-correct texture mapping. Doesn't actually work. Breaks 8-bit lighting.
    cube->material->flags |= BR_MATF_DITHER;      // BRenderModern: Dithering.
    cube->material->flags |= BR_MATF_SMOOTH;      // BRenderModern: Makes lighting look _much_ better.
    // BRenderModern: cube->material->flags |= BR_MATF_DISABLE_COLOUR_KEY;  // Not supported by software.
    cube->material->opacity = 255; // BRenderModern: < 255 selects screendoor renderer
