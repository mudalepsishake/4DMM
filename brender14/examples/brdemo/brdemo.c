...
/* BRenderModern: begin hook */
void _BrBeginHook(void) // BRenderModern: NOLINT(*-reserved-identifier)
{
...
/* BRenderModern: end hook */
void _BrEndHook(void) // BRenderModern: NOLINT(*-reserved-identifier)
{
}
...
/* BRenderModern: FIXME: de-global this */
...
static br_error create_window(br_demo *demo, const br_demo_run_args *args)
{
    ...
    err = BrDevBeginVar(&demo->_screen,              "SDL3",
                        ...
                        BRT_OPENGL_DEVICE_NAME_CSTR, args->opengl_device_name, /* BRenderModern: NB: this is optional */
                        BR_NULL_TOKEN);
...
int BrDemoRunArg(const br_demo_dispatch *dispatch, const br_demo_run_args *args)
{
    ...
    /* BRenderModern: FIXME: hack to work around no args in the find hooks. */
    current_demo = demo;
