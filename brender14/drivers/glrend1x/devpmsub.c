...
{
    /* BRenderModern: Remap us back to top-left at (0, 0). */
    p.x = p.x + self->pm_origin_x;
    p.y = p.y + self->pm_origin_y;

    /* BRenderModern: Map to our parent's (0, 0) by undoing the base. */
    p.x = p.x + self->pm_base_x;
    p.y = p.y + self->pm_base_y;

    /* BRenderModern: Map to our parent's space. */
...
static void BR_CMETHOD_DECL(br_device_pixelmap_sub_gl, free)(br_object *_self)
{
    ...
    DevicePixelmapGL1xDecRef(self->asSub.parent);
