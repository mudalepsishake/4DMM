...
#define DEVICE_TITLE   "OpenGL v1.x"
...
static br_tv_template_entry deviceTemplateEntries[] = {
     ...
     */
    {BRT(OPENGL_VERSION_MAJOR_U8), 0,                BRTV_QUERY | BRTV_ALL,            BRTV_CONV_DIRECT, 1                               },
    {BRT(OPENGL_VERSION_MINOR_U8), 0,                BRTV_QUERY | BRTV_ALL,            BRTV_CONV_DIRECT, 2                               },
    {BRT(OPENGL_PROFILE_T),        0,                BRTV_QUERY | BRTV_ALL,            BRTV_CONV_DIRECT, BRT_OPENGL_PROFILE_COMPATIBILITY},
};
#undef F
#undef A
...
br_device *DeviceGL1xAllocate(const char *identifier, const char *arguments)
{
    ...
    if((self->output_facility = OutputFacilityGL1xInit(self)) == NULL) {
