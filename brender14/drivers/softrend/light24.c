...
{
    comp[C_R] = BR_MUL(self->state.cache.comp_scales[C_R], BR_CONST_DIV(BrIntToScalar(BR_RED(colour)), 256)) + self->state.cache.comp_offsets[C_R];

    comp[C_G] = BR_MUL(self->state.cache.comp_scales[C_G], BR_CONST_DIV(BrIntToScalar(BR_GRN(colour)), 256)) + self->state.cache.comp_offsets[C_G];

    comp[C_B] = BR_MUL(self->state.cache.comp_scales[C_B], BR_CONST_DIV(BrIntToScalar(BR_BLU(colour)), 256)) + self->state.cache.comp_offsets[C_B];
}
...
br_scalar lightVolumeAttenuation(br_vector3 *p, br_light_volume *volume)
{
    ...
    return BR_SCALAR(1.0) - BR_DIV(atten_dist, volume->falloff_distance); // BRender: ** CACHE 1/falloff **
}
