 ...
 */
static int ClipFaceToPlane(brp_vertex *vp, brp_vertex *verts_out, int num_in, br_vector4 *plane, int cmask)
{
    ...
    tu = -BR_MAC4(plane->v[0], up->comp[C_X], plane->v[1], up->comp[C_Y], plane->v[2], up->comp[C_Z], plane->v[3], up->comp[C_W]);

    for(; num_in--; up = vp, tu = tv, vp++) {

        tv = -BR_MAC4(plane->v[0], vp->comp[C_X], plane->v[1], vp->comp[C_Y], plane->v[2], vp->comp[C_Z], plane->v[3], vp->comp[C_W]);
 ...
 */
static int ClipFaceToPlus1(brp_vertex *vp, brp_vertex *verts_out, int num_in, int axis, int cmask)
{
    brp_vertex *wp      = verts_out;
    int         num_out = 0;
 ...
 * Special case of sign = -1, k = 1.0
 */
static int ClipFaceToMinus1(brp_vertex *vp, brp_vertex *verts_out, int num_in, int axis, int cmask)
{
    brp_vertex *wp      = verts_out;
    int         num_out = 0;
...
brp_vertex *FaceClip(br_renderer *self, brp_vertex *clip_in, br_uint_32 mask, br_uint_32 codes, int n, int *n_out)
{
    ...
    brp_vertex       *cp_in = clip_in, *cp_out = clip_poly_1;
    int               c;
 ...
 * of the line remains
 */
br_boolean ClipLineToPlane(brp_vertex *in, brp_vertex *out, br_vector4 *plane, int cmask)
{
    br_scalar  t, tu, tv;
    int        m;
    br_scalar *usp, *vsp, *wsp;

    tu = -BR_MAC4(plane->v[0], in[0].comp[C_X], plane->v[1], in[0].comp[C_Y], plane->v[2], in[0].comp[C_Z], plane->v[3], in[0].comp[C_W]);

    tv = -BR_MAC4(plane->v[0], in[1].comp[C_X], plane->v[1], in[1].comp[C_Y], plane->v[2], in[1].comp[C_Z], plane->v[3], in[1].comp[C_W]);
...
br_boolean ClipLine(br_renderer *self, brp_vertex *out, brp_vertex *v0, brp_vertex *v1, br_uint_32 mask, br_uint_32 codes)
{
    ...
    brp_vertex  cv0[2], cv1[2];
    brp_vertex *cp_in = cv0, *cp_out = cv1, *cp_tmp;
    int         c;
