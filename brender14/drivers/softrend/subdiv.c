...
{
    // BRenderModern: ;assumes edx contains outcode flags,esi target for discriptor edi,reg0,reg1
    // BRenderModern: ;14 cycles - (at least 2.5 wasted cycles)
    br_uint_32 reg0, reg1;
    br_uint_32 esi, edi;

    // BRenderModern: mov reg0,rValue
    BrMemCpy(&reg0, &rValue, sizeof(br_uint_32));

    // BRenderModern: mov esi,080000000h
    esi = 0x80000000;

    // BRenderModern: and esi,reg0
    esi &= reg0;

    // BRenderModern: mov reg1,lValue
    BrMemCpy(&reg1, &lValue, sizeof(br_uint_32));

    // BRenderModern: mov edi,080000000h
    edi = 0x80000000;
    // BRenderModern: and reg0,07fffffffh
    reg0 &= 0x07fffffff;

    // BRenderModern: shr esi,1
    esi >>= 1;
    // BRenderModern: and edi,reg1
    edi &= reg1;

    // BRenderModern: or esi,edi
    esi |= edi;

    // BRenderModern: mov edi,reg0
    edi = reg0;

    // BRenderModern: and reg1,07fffffffh
    reg1 &= 0x07fffffff;

    // BRenderModern: sub edi,reg1
    edi -= reg1;
    // BRenderModern: sub reg1,reg0
    reg1 -= reg0;

    // BRenderModern: shr edi,3
    edi >>= 3;
    // BRenderModern: and reg1,080000000h
    reg1 &= 0x80000000;

    // BRenderModern: shr reg1,2
    reg1 >>= 2;
    // BRenderModern: or esi,edi
    esi |= edi;

    // BRenderModern: or esi,reg1
    esi |= reg1;
