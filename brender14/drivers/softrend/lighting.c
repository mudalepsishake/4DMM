...
static br_tv_template_entry geometryLightingTemplateEntries[] = {
                                                                   ...
                                                                   int coutstride, int nvertices)
{
    ...
    r = PrimitiveStateRenderBegin(renderer->state.pstate, &rend.block, &rend.block_changed, &rend.range_changed, 0, BRT_TRIANGLE);
    ...
    for(j = 0; j < nvertices; j++) {
        for(i = 0; i < renderer->state.cache.nvertex_fns; i++)
            renderer->state.cache.vertex_fns[i](renderer, (br_vector3 *)points, &map, (br_vector3 *)normals, *colour_in, comp);
        for(i = 0; i < renderer->state.cache.nconstant_fns; i++)
            renderer->state.cache.constant_fns[i](renderer, (br_vector3 *)points, &map, (br_vector3 *)normals, *colour_in, comp);
