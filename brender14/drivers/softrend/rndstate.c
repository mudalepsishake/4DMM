...
{
    br_error        r;
    state_all      *sp = &self->state;
    br_tv_template *tp = FindStateTemplate(self, &sp, part, index);
    br_uint_32      m  = 0;
...
br_error BR_CMETHOD_DECL(br_renderer_soft, partQueryAll)(br_renderer *self, br_token part, br_int_32 index, br_token_value *buffer, br_size_t buffer_size)
{
    br_error        r;
    state_all      *sp = &self->state;
    br_tv_template *tp = FindStateTemplate(self, &sp, part, index);
