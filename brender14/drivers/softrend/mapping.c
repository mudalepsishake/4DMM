...
#define APPLY_UV(du, dv, su, sv)                                                                                       \
    do {                                                                                                               \
        du = BR_ADD(BR_MAC2(self->state.cache.map_transform.m[0][0], su, self->state.cache.map_transform.m[1][0], sv), \
                    self->state.cache.map_transform.m[2][0]);                                                          \
        dv = BR_ADD(BR_MAC2(self->state.cache.map_transform.m[0][1], su, self->state.cache.map_transform.m[1][1], sv), \
 ...
 */
void SURFACE_CALL SurfaceMapEnvironmentLocal(br_renderer *self, br_vector3 *p, br_vector2 *map, br_vector3 *normal, br_colour colour, br_scalar *comp)
{
    br_vector3 eye;
    br_vector2 r;
