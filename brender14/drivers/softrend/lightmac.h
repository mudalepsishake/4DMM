...
#define CLAMP_SCALE(c)                                                                                                \
    do {                                                                                                              \
        if(BR_SCALAR_GE_1(comp[(c)]))                                                                                 \
            ...
            comp[(c)] = BR_MUL(self->state.cache.comp_scales[(c)], comp[(c)]) + self->state.cache.comp_offsets[(c)];  \
    } while(0);
 ...
 */
#define CALCULATE_DIRN_NORM()                  \
    do {                                       \
        BrVector3Normalise(&dirn_norm, &dirn); \
    } while(0)

#define CALCULATE_DIRN_NORM_DIST()            \
    do {                                      \
        br_scalar s;                          \
...
#define CALCULATE_DIRN_NORM_DIST_CUTOFF(cutoff) \
    do {                                        \
        br_scalar s;                            \
...
#define CALCULATE_DIST_CUTOFF(cutoff)         \
    do {                                      \
        dist = BrVector3LengthSquared(&dirn); \
        if(BR_SCALAR_GT(dist, cutoff))        \
            return;                           \
...
#define CALCULATE_ATTENUATION()                                                                                                       \
    do {                                                                                                                              \
        br_scalar dist2 = DIST_SQR(dist);                                                                                             \
        attn            = BR_RCP(alp->s->attenuation_c + BR_MUL(dist, alp->s->attenuation_l) + BR_MUL(dist2, alp->s->attenuation_q)); \
    } while(0)

#define MULTIPLY_ATTENUATION(l)                                                                                                         \
    do {                                                                                                                                \
        br_scalar dist2 = DIST_SQR(dist);                                                                                               \
        attn = BR_MUL((l), BR_RCP(alp->s->attenuation_c + BR_MUL(dist, alp->s->attenuation_l) + BR_MUL(dist2, alp->s->attenuation_q))); \
    } while(0)

#define CALCULATE_ATTENUATION_LINEAR()                                              \
    do {                                                                            \
        attn = BR_RCP(alp->s->attenuation_c + BR_MUL(dist, alp->s->attenuation_l)); \
    } while(0)

#define MULTIPLY_ATTENUATION_LINEAR(l)                                                   \
    do {                                                                                 \
        attn = BR_DIV((l), alp->s->attenuation_c + BR_MUL(dist, alp->s->attenuation_l)); \
    } while(0)

#define CALCULATE_ATTENUATION_RADII()                                 \
    do {                                                              \
        if(BR_SCALAR_LT(dist, alp->s->radius_inner))                  \
            attn = alp->intensity;                                    \
        else                                                          \
            attn = BR_MUL(alp->falloff, dist - alp->s->radius_outer); \
    } while(0)

#define MULTIPLY_ATTENUATION_RADII(l)                                              \
    do {                                                                           \
        if(BR_SCALAR_LT(dist, alp->s->radius_inner))                               \
            attn = BR_MUL((l), alp->intensity);                                    \
        else                                                                       \
            attn = BR_MUL((l), BR_MUL(alp->falloff, dist - alp->s->radius_outer)); \
    } while(0)
...
#define SPOT_FALLOFF(l) ((dot_spot < alp->s->spot_inner) ? BR_MUL((l), BR_MUL(alp->s->spot_outer - dot_spot, alp->spot_falloff)) : (l))

#define MULTIPLY_SPOT_ATTENUATION(l)                                                                                                       \
    do {                                                                                                                                   \
        attn = (BR_SCALAR_LT(dot_spot, alp->s->spot_inner) ? BR_MUL((l), BR_MUL(alp->s->spot_outer - dot_spot, alp->spot_falloff)) : (l)); \
    } while(0)

/* BRender:
 * Checking light volume
 */
#define CHECK_LIGHT_VOLUME()                      \
    do {                                          \
        if(!pointInLightVolumes(p, &alp->volume)) \
            return;                               \
    } while(0)

#define CALCULATE_LIGHT_VOLUME_ATTENUATION()            \
    do {                                                \
...
#define MULTIPLY_LIGHT_VOLUME_ATTENUATION(l)                         \
    do {                                                             \
 ...
 */
#define ACCUMULATE_AMBIENT_RGB(l)             \
    do {                                      \
...
#define ACCUMULATE_DIFFUSE_RGB(l)                 \
    do {                                          \
 ...
 */
#define ACCUMULATE_SPECULAR_RGB(l)                      \
    do {                                                \
