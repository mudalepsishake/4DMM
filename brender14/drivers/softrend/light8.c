...
{
    comp[C_I] = BR_MUL(self->state.cache.comp_scales[C_I], BR_CONST_DIV(BrIntToScalar(BR_ALPHA(colour)), 256)) +
...
void ActiveLightAccumulateIndexSet(struct active_light *alp)
{
    ...
    switch(alp->type) {
        ...
        case BRT_POINT:
            ...
            if(alp->s->attenuation_l == BR_SCALAR(0.0) && alp->s->attenuation_q == BR_SCALAR(0.0))
