...
typedef struct rend_block {
    ...
    // BRender:        br_int_32 nvisible_faces;
    ...
    {                                                                                                                        \
        ...
        if(scache.user_clip_active) {                                                                                        \
            ...
            for(c = 0; c < MAX_STATE_CLIP_PLANES; c++)                                                                       \
                if(renderer->state.clip[c].type == BRT_PLANE && BrVector4Dot((screen), &renderer->state.clip[c].plane) < S0) \
...
#define PROJECT_VERTEX(tvp, sx, sy, sz, sw)                                                                                              \
    {                                                                                                                                    \
        ...
        (tvp)->comp[C_SX] = renderer->state.cache.comp_offsets[C_SX] + BR_MUL(renderer->state.cache.comp_scales[C_SX], BR_MUL((sx), q)); \
        (tvp)->comp[C_SY] = renderer->state.cache.comp_offsets[C_SY] + BR_MUL(renderer->state.cache.comp_scales[C_SY], BR_MUL((sy), q)); \
        (tvp)->comp[C_SZ] = renderer->state.cache.comp_offsets[C_SZ] + BR_MUL(renderer->state.cache.comp_scales[C_SZ], BR_MUL((sz), q)); \
    }
...
#define PROJECT_VERTEX_WRITE_Q(tvp, sx, sy, sz, sw)                                                          \
    {                                                                                                        \
        ...
        (tvp)->comp[C_SX] = renderer->state.cache.comp_offsets[C_SX] +                                       \
                            BR_MUL(renderer->state.cache.comp_scales[C_SX], BR_MUL((sx), (tvp)->comp[C_Q])); \
        (tvp)->comp[C_SY] = renderer->state.cache.comp_offsets[C_SY] +                                       \
                            BR_MUL(renderer->state.cache.comp_scales[C_SY], BR_MUL((sy), (tvp)->comp[C_Q])); \
        (tvp)->comp[C_SZ] = renderer->state.cache.comp_offsets[C_SZ] +                                       \
                            BR_MUL(renderer->state.cache.comp_scales[C_SZ], BR_MUL((sz), (tvp)->comp[C_Q])); \
    }
...
#define TRANSFORM_VERTEX(dest, src, mat)                                                                                                \
    {                                                                                                                                   \
        (dest)->v[X] = BR_MAC3((src)->v[X], (mat)->m[0][0], (src)->v[Y], (mat)->m[1][0], (src)->v[Z], (mat)->m[2][0]) + (mat)->m[3][0]; \
                                                                                                                                        \
        (dest)->v[Y] = BR_MAC3((src)->v[X], (mat)->m[0][1], (src)->v[Y], (mat)->m[1][1], (src)->v[Z], (mat)->m[2][1]) + (mat)->m[3][1]; \
                                                                                                                                        \
        (dest)->v[Z] = BR_MAC3((src)->v[X], (mat)->m[0][2], (src)->v[Y], (mat)->m[1][2], (src)->v[Z], (mat)->m[2][2]) + (mat)->m[3][2]; \
                                                                                                                                        \
        (dest)->v[W] = BR_MAC3((src)->v[X], (mat)->m[0][3], (src)->v[Y], (mat)->m[1][3], (src)->v[Z], (mat)->m[2][3]) + (mat)->m[3][3]; \
    }
...
#define UPDATE_BOUNDS(tv)                       \
    {                                           \
        if((tv)->comp[C_SX] > scache.max.v[X])  \
            scache.max.v[X] = (tv)->comp[C_SX]; \
        if((tv)->comp[C_SX] < scache.min.v[X])  \
            scache.min.v[X] = (tv)->comp[C_SX]; \
        if((tv)->comp[C_SY] > scache.max.v[Y])  \
            scache.max.v[Y] = (tv)->comp[C_SY]; \
        if((tv)->comp[C_SY] < scache.min.v[Y])  \
            scache.min.v[Y] = (tv)->comp[C_SY]; \
    }
