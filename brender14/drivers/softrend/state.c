 ...
 */
#define TM_PART           0x00001 /* BRender: set timestamp for whole part								*/
#define TM_INDEX          0x00002 /* BRender: set timestamp for indexed part							*/
#define TM_V2S_HINT       0x00004 /* BRender: set timestamp for matrix.view_to_screen_hint				*/
#define TM_M2V            0x00008 /* BRender: set timestamp for matrix.model_to_view					*/
#define TM_V2S            0x00010 /* BRender: set timestamp for matrix.view_to_screen					*/
#define TM_V2E            0x00020 /* BRender: set timestamp for matrix.view_to_environment				*/
#define TM_COPY_CACHE     0x00040 /* BRender: set timestamp for anything that affects the copied cache	*/
#define TM_CACHE          0x00080 /* BRender: set timestamp for anything that affects the static cache	*/

#define TM_CLEAR_M2V_HINT 0x00100 /* BRender: set M2V_HINT to NONE										*/
#define TM_CLEAR_V2S_HINT 0x00200 /* BRender: set V2S_HINT to NONE										*/

#define TM_INVALID_PS     0x01000 /* BRender: set Update Per Scene										*/
#define TM_INVALID_PM     0x02000 /* BRender: set Update Per Model										*/
#define TM_INVALID_V2M    0x04000 /* BRender: set Update View To Model									*/
#define TM_INVALID_M2S    0x08000 /* BRender: set Update Model To Screen								*/
#define TM_INVALID_CC     0x10000 /* BRender: set Update Copied Cache									*/

/** BRender: ************************************************************************
 ** Culling
 **/

#define F(f) offsetof(state_all, f)
 ...
 ** Light
 **/
static br_error BR_CALLBACK customLightingVolumeSet(void *block, const br_value *pvalue, const br_tv_template_entry *tep);

static const br_tv_custom customLightingVolumeConv = {
    .query      = NULL,
...
static const struct state_enable partEnableDefault = {
    .flags = ENBL_TRANSFORM | ENBL_CLIPPING | ENBL_RENDER,
};
...
static const struct state_bounds partBoundsDefault = {
    .min = {BR_SCALAR_MAX, BR_SCALAR_MAX},
    .max = {BR_SCALAR_MIN, BR_SCALAR_MIN},
};
    ...
    if(volume->regions != NULL) {
        ...
        if(new_volume->regions != NULL)
            ...
            for(i = 0; i < volume->nregions; i++) {
                ...
                new_volume->regions[i].planes = BrResAllocate(
                    new_volume->regions, sizeof(*new_volume->regions[i].planes) * volume->regions[i].nplanes, BR_MEMORY_OBJECT_DATA);
                ...
                BrMemCpy(new_volume->regions[i].planes, volume->regions[i].planes,
                         sizeof(*new_volume->regions[i].planes) * volume->regions[i].nplanes);
            }
