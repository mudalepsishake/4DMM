...
static struct br_tv_template_entry devicePixelmapTemplateEntries[] = {
    ...
    // BRenderModern:	{BRT(PIXEL_CHANNELS_TL),0,					BRTV_QUERY | BRTV_ALL,	BRTV_CONV_CUSTOM },
    // BRenderModern:	{BRT(INDEXED_B,)		F(indexed),			BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY },
...
static void BR_CMETHOD_DECL(br_device_pixelmap_vesa, free)(br_object *_self)
{
    ...
    if(self->restore_mode) {
        // BRender: VESAModeSet(self->original_mode);
...
static br_error BR_CMETHOD_DECL(br_device_pixelmap_vesa, synchronise)(br_device_pixelmap *self, br_token sync_type, br_boolean block)
{
    ...
    /* BRenderModern: http://www.techhelpmanual.com/899-enhanced_graphics_adapter_i_o_ports.html */
