...
static struct br_tv_template_entry outputFacilityTemplateEntries[] = {
    ...
    // BRenderModern:	{BRT(PIXEL_CHANNELS_I32),F(channels_mask),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, 0},
    // BRenderModern:	{BRT(PIXEL_CHANNELS_TL), F(channels_list),		BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, 0},

    {BRT(INDEXED_B),        F(indexed),     BRTV_QUERY | BRTV_ALL, BRTV_CONV_COPY,    0},

    // BRenderModern:	{BRT(MEMORY_MAPPED_B), 	F(),				BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, 0},

    {BRT(PIXELMAP_MAX_I32), 0,              BRTV_QUERY | BRTV_ALL, BRTV_CONV_DIRECT,  1},
    {BRT(CLUT_MAX_I32),     0,              BRTV_QUERY | BRTV_ALL, BRTV_CONV_DIRECT,  0},

    // BRender:	{BRT(VIDEO_MEMORY_U32), F(),				BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, },
    // BRender:	{BRT(TEXTURE_MEMORY_U32), F(),				BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, },
    // BRender:	{BRT(HOST_MEMORY_U32), F(),					BRTV_QUERY | BRTV_ALL,	BRTV_CONV_COPY, },
