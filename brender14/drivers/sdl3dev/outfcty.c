...
static struct br_tv_template_entry outputFacilityTemplateEntries[] = {
    ...
    /* BRenderModern: Unused, just required for matching. */
    ...
    {BRT(OPENGL_DEVICE_NAME_CSTR), (br_uintptr_t)NULL,     BRTV_QUERY | BRTV_ALL, BRTV_CONV_DIRECT, 0},
...
static struct br_tv_template_entry pixelmapNewTemplateEntries[] = {
    ...
    {BRT(OPENGL_DEVICE_NAME_CSTR), F(opengl_device_name), BRTV_SET, BRTV_CONV_COPY, 0                            },
};
#undef F
...
const static struct {
    ...
    // BRenderModern: {.format = SDL_PIXELFORMAT_INDEX4MSB, .bpp = 4,  .type = BR_PMT_INDEX_4  }, /* Untested */
 ...
 *
 */
static br_error BR_CMETHOD_DECL(br_output_facility_sdl3, pixelmapNew)(br_output_facility *self, br_device_pixelmap **ppmap, br_token_value *tv)
{
    ...
    if(is_window) {
        if(is_new) {
            ...
            /* BRenderModern: Not valid to set the pixel format on an existing window. */
            if(pt.pixel_type != BR_PMT_MAX)
                return BRE_FAIL;

            /* BRenderModern: Update the size, if specified. */
            if(pt.width > 0 && pt.height > 0)
                SDL_SetWindowSize(pt.window, pt.width, pt.height);

            /* BRenderModern: Update the title, if specified. */
            if(pt.title != NULL)
                SDL_SetWindowTitle(pt.window, pt.title);
        }

        /* BRenderModern: Get the window surface. */
    ...
    if((pm = DevicePixelmapSDL3Allocate(self->device, self, window, surface, is_new)) == NULL) {
        /* BRenderModern: NB: This will window/surface cleanup. */
        return BRE_FAIL;
    }
...
static br_error BR_CMETHOD_DECL(br_output_facility_sdl3, clutNew)(br_output_facility *self, struct br_device_clut **pclut, br_token_value *tv)
{
    (void)self;
    (void)pclut;
    (void)tv;
