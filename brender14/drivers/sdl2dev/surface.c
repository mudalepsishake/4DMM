...
br_clip_result DevicePixelmapSDL2LineClip(SDL_Point *s_out, SDL_Point *e_out, const br_point *s_in, const br_point *e_in, const br_pixelmap *pm)
{
    br_clip_result r;
    br_point       stmp, etmp;
...
br_clip_result DevicePixelmapSDL2RectangleClipTwo(SDL_Rect *r_out, SDL_Point *p_out, const br_rectangle *r_in, const br_point *p_in,
...
br_error DeviceSDL2SetPaletteFromMap(SDL_Palette *pal, const br_pixelmap *map)
{
    ...
    switch(map->type) {
        ...
        case BR_PMT_RGBA_8888:
            ...
            break;
        default:
            /* BRenderModern: Have never seen CLUTs that aren't these formats. */
            return BRE_FAIL;
    }
...
static int xblit(SDL_Surface *src, const SDL_Rect *sr, SDL_Surface *dst, SDL_Rect *dr, br_sdl_blit_cbfn blit)
{
     ...
     */
    if(SDL_ISPIXELFORMAT_INDEXED(src->format->format) && (dst->format->format == SDL_PIXELFORMAT_XRGB8888 || blit == SDL_BlitScaled)) {
        int          r;
        SDL_Surface *newsrc;
...
br_error DevicePixelmapSDL2BlitSurface(br_pixelmap *src, SDL_Rect *sr, br_pixelmap *dst, SDL_Rect *dr, br_sdl_blit_cbfn blit)
{
    ...
    // BRenderModern: TODO: handle colour keyed copy
