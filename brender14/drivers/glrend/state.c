...
{
    /* BRenderModern: Restrict the copy mask to the valid parts. */
    mask &= src->valid;

    /* BRenderModern: Merge the valid mask into the destination. */
    dst->valid |= mask;
...
br_boolean StateGLPush(state_all *state, uint32_t mask)
{
    ...
    // BRenderModern: StateGLCopy(state->current, old, mask);
    *state->current = *old;
    return BR_TRUE;
}
...
br_boolean StateGLPop(state_all *state, uint32_t mask)
{
    ...
    // BRenderModern: StateGLCopy(&state->current, sp, mask);
    return BR_TRUE;
}
...
// BRenderModern: d3drend:state.c:452 void TemplateActions(struct state_all *state, br_token part, br_int_32 index, br_uint_32 mask)
void StateGLTemplateActions(state_all *state, uint32_t mask)
{
    // BRenderModern: if(mask & TM_CLEAR_M2V_HINT)
    // BRenderModern:     state->current.matrix.model_to_view_hint = BRT_NONE;
    //
    // BRenderModern: if(mask & TM_CLEAR_V2S_HINT)
    // BRenderModern:     state->current.matrix.view_to_screen_hint = BRT_NONE;
}

// BRenderModern: d3drend:state.c:351 struct br_tv_template * FindStateTemplate(struct br_renderer *self, struct state_all **state,
// BRenderModern: br_token part, br_int_32 index)
