...
static br_error BR_CMETHOD_DECL(br_device_clut_gl, entrySetMany)(br_device_clut *self, br_int_32 index, br_int_32 count, br_colour *entries)
{
    ...
    if(DeviceGLPixelmapToExistingGLTexture(gl, self->gl_tex, self->pm) != BRE_OK)
        return BRE_FAIL;

    DeviceGLObjectLabelF(gl, GL_TEXTURE, self->gl_tex, BR_GLREND_DEBUG_INTERNAL_PREFIX "%s", self->identifier);
    return BRE_OK;
}
