...
static void get_shader_variables(br_gl_main_shader *self, const GladGLContext *gl)
{
    ...
    self->uniforms.shadow_texture         = gl->GetUniformLocation(self->program, "shadow_texture");
    self->uniforms.shadow_owner_texture   = gl->GetUniformLocation(self->program, "shadow_owner_texture");
    self->uniforms.shadow_blocker_texture = gl->GetUniformLocation(self->program, "shadow_blocker_texture");
    self->uniforms.shadow_detail_texture  = gl->GetUniformLocation(self->program, "shadow_detail_texture");
...
br_boolean ShaderGLMainCompile(br_gl_main_shader *self, const GladGLContext *gl, const char *vert_path, const char *frag_path)
{
    ...
    self->shadow_texture_binding         = 2;
    self->shadow_owner_texture_binding   = 3;
    self->shadow_blocker_texture_binding = 4;
    self->shadow_detail_texture_binding  = 5;
...
#undef _MAX
    ...
    /* BRenderModern: br_model_state, br_scene_state */
...
br_boolean ShaderGLShadowCompile(br_gl_shadow_shader *self, const GladGLContext *gl)
{
    ...
    self->block_binding_scene = 1;
    self->block_binding_model = 2;
    self->base_depth_texture_binding = 6;

    vert = DeviceGLLoadAndCompileShader(gl, GL_VERTEX_SHADER, NULL, g_ShadowVertexShader, sizeof(g_ShadowVertexShader));
    if(!vert)
        goto vert_failed;

    frag = DeviceGLLoadAndCompileShader(gl, GL_FRAGMENT_SHADER, NULL, g_ShadowFragmentShader, sizeof(g_ShadowFragmentShader));
    if(!frag)
        goto frag_failed;

    self->program = DeviceGLCreateAndCompileProgram(gl, vert, frag);
    if(!self->program)
        goto prog_failed;

    DeviceGLObjectLabel(gl, GL_SHADER, vert, BR_GLREND_DEBUG_INTERNAL_PREFIX "shadow:shader:vertex");
    DeviceGLObjectLabel(gl, GL_SHADER, frag, BR_GLREND_DEBUG_INTERNAL_PREFIX "shadow:shader:fragment");
    DeviceGLObjectLabel(gl, GL_PROGRAM, self->program, BR_GLREND_DEBUG_INTERNAL_PREFIX "shadow:shader:program");

    self->block_index_scene = gl->GetUniformBlockIndex(self->program, "br_scene_state");
    self->block_index_model = gl->GetUniformBlockIndex(self->program, "br_model_state");
    gl->UniformBlockBinding(self->program, self->block_index_scene, self->block_binding_scene);
    gl->UniformBlockBinding(self->program, self->block_index_model, self->block_binding_model);
    self->uniform_base_depth_texture = gl->GetUniformLocation(self->program, "shadowBaseDepthTexture");
    self->uniform_layer_mode = gl->GetUniformLocation(self->program, "shadowLayerMode");
