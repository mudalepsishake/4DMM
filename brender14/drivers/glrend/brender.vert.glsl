...
layout(location=10) out vec3 shadowPosition;
layout(location=11) out vec3 shadowNormal;
...
void main()
{
    ...
    shadowPosition = (shadow_model_to_light * pos).xyz;
    shadowNormal = normalize(transpose(inverse(mat3(shadow_model_to_light))) * aNormal);

    vertexLighting = mat3(0);

    if(shading_mode == SHADING_MODE_GOURAUD) {
        accumulateLights(position.xyz, normal, shadowPosition, shadowNormal,
                         vertexLighting[0], vertexLighting[1], vertexLighting[2]);
    }
