...
/* 4DMM v271: allow the shared shadow code to use fragment derivatives. */
#define BRENDER_FRAGMENT_STAGE 1
...
layout(location=10) in vec3 shadowPosition;
layout(location=11) in vec3 shadowNormal;
...
void main()
{
    ...
    if(shading_mode == SHADING_MODE_PHONG) {
        accumulateLights(position.xyz, normal, shadowPosition, normalize(shadowNormal), lightA, lightD, lightS);
    }
