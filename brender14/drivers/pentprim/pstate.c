...
static br_error BR_CALLBACK customInputSet(void *block, const br_value *pvalue, const struct br_tv_template_entry *tep)
{
	...
	if(inputSet(
		...
		((struct br_tv_template_entry*)tep)->mask = 1; /* BRenderModern: FIXME: casting away const */
	else
		((struct br_tv_template_entry*)tep)->mask = 0; /* BRenderModern: FIXME: casting away const */
