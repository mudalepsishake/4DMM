...
static struct {
    int type;                                                                   /* BRender: pixelmap type */
    br_colour (*function)(br_pixelmap *pm, int x, int y, int offset, int byte); /* BRender: function to read pixel from pixelmap */
} GetPixelTypes[] = {
...
static struct {
    int type;                                                                                /* BRender: pixelmap type */
    void (*function)(br_pixelmap *pm, br_colour colour, int x, int y, int offset, int byte); /* BRender: function to save pixel to pixelmap */
} PutPixelTypes[] = {
    {BR_PMT_INDEX_8,   NULL             },
...
int PalettePresent(br_pixelmap *pm);
...
void DoQuantize(br_pixelmap **mtable, int nmaps, int base, int range, br_pixelmap *palette)
{
    ...
    for(i = 0; i < nmaps; i++) {
#ifdef VERBOSE
        fprintf(stderr, "Quantizing '%s' to palette '%s' using %d colours from the range %d-%d\n",
...
void DoRemap(br_pixelmap **mtable, int nmaps, int base, int range)
{
    ...
    for(i = 0; i < nmaps; i++) {
#ifdef VERBOSE
        fprintf(stderr, "Remapping '%s' to the range %d-%d", mtable[i]->identifier, base, base + range - 1);
#endif
        /* BRender:
...
void DoRemapWithSupermaps(br_pixelmap **mtable, int nmaps, br_pixelmap **smtable, int nsmaps, int smap_power, int base, int range)
{
    ...
    for(i = 0; i < nsmaps; i++) {
#ifdef VERBOSE
        fprintf(stderr, "Remapping supermap '%s' to the range %d-%d", smtable[i]->identifier, base, base + range - 1);
#endif
        /* BRender:
         * only add pixel data if there is a palette to find rgb
         */
