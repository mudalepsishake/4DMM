...
Output_Types OutputFileTypes[] = {
    ...
    {"palette",  T_PALETTE }, /* BRender: save any palette 		: pixelmap->map			*/
    {"pal",      T_PALETTE },

    {"image",    T_IMAGE   }, /* BRender: save image without palette	: pixelmap, pixelmap->map=NULL	*/

    {"pixelmap", T_PIXELMAP}, /* BRender: save pixelmap with palette	: pixelmap			*/
    {"pix",      T_PIXELMAP}, /* BRender: save pixelmap with palette	: pixelmap			*/

    {"targa",    T_TARGA   }, /* BRender: save as (uncompressed) targa					*/
    {"tga",      T_TARGA   },
};
...
void T_OutputPixelmap(char *filename, br_uint_32 type)
{
    ...
    switch(type) {
        ...
        case T_TARGA: {
            ...
            if(cbfn_command->nmaps == 1) {
                ...
                T_Log("Saved '%s' (%s) as '%s'\n", cbfn_command->map_table[0]->identifier,
                      T_EncodePixelmapType(cbfn_command->map_table[0]->type), name);
            } else
                for(i = 0; i < cbfn_command->nmaps; i++) {
                    ...
                    T_Log("Saved '%s' (%s) as '%s'\n", cbfn_command->map_table[i]->identifier,
                          T_EncodePixelmapType(cbfn_command->map_table[i]->type), name);
                }
        ...
        case T_IMAGE:
             ...
             */
            cbfn_command->map_table = BrResAllocate(cbfn_command, sizeof(*cbfn_command->map_table) * cbfn_command->nmaps,
        ...
        case T_PALETTE:
            ...
            cbfn_command->map_table = BrResAllocate(cbfn_command, sizeof(*cbfn_command->map_table) * cbfn_command->npals,
                                                    BR_MEMORY_APPLICATION);
...
save_pix:
            ...
            if(n == 0)
                ...
                for(i = 0; i < cbfn_command->nmaps - 1; i++) {
                    ...
                    T_Log("'%s' (%d,%d  %s)", cbfn_command->map_table[i]->identifier, cbfn_command->map_table[i]->width,
                          cbfn_command->map_table[i]->height, T_EncodePixelmapType(cbfn_command->map_table[i]->type));
                ...
                T_Log(" to '%s' (%d pixelmap%s", filename, cbfn_command->nmaps, (cbfn_command->nmaps == 1) ? ")\n" : "s)\n");
            }
