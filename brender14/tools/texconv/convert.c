...
{
    /* BRender: colour format R G B R G B R G B */
...
br_colour T_Read_Index_8(char *pixels, br_float fract)
{
    ...
    if(LEAVE_TRANSPARENT) {
        // BRender: If Colour 0 then Transparent
        if(index == 0)
...
{
    /* BRender: colour format X R G B X R G B X R G B */
...
{
    /* BRender: colour format R G B R G B R G B  - 2 bytes per pixel */
...
{
    /* BRender: colour format B G R B G R B G R  - 2 bytes per pixel */
...
{
    /* BRender: colour format B G R - 2 bytes per pixel */
    br_uint_16 temp_colour;
...
{
    /* BRender: colour format R G B A R G B A R G B A - 2 bytes per pixel */
...
br_pixelmap *T_ConvertIndexedPixelmap(br_pixelmap *item, t_pixelmap_cbfn_info *cbfn_command)
{
    ...
    if(cbfn_command->type == T_CBFN_REMAP_PIXELMAP) {
        ...
        if((cbfn_command->base < 0) || (cbfn_command->base > cbfn_command->height - 1))
            BR_ERROR3("Index base value out of range 0-%d (%d) for type %s", cbfn_command->height - 1,
                      cbfn_command->base, T_EncodePixelmapType(pm->type));
        ...
        if((cbfn_command->base + cbfn_command->range - 1) > (cbfn_command->height - 1))
            BR_ERROR2("Too many colours specified for indexed pixelmap of type %s (%d)", T_EncodePixelmapType(pm->type),
                      cbfn_command->base + cbfn_command->range - 1);
    }
     ...
     */
    for(y = 0, s_fract = (temp_pm->base_x * s_step); y < temp_pm->height;
        y++, s_pixels += temp_pm->row_bytes, s_fract = temp_pm->base_x * s_step)
     ...
     */
    for(y = 0, s_fract = temp_pm->base_x * s_step, d_fract             = pm->base_x * d_step; y < temp_pm->height; y++,
    s_pixels += temp_pm->row_bytes, d_pixels += pm->row_bytes, s_fract = temp_pm->base_x * s_step, d_fract = pm->base_x * d_step)
