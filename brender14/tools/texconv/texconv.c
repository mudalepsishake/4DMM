...
int CurrentPixelmaps(br_pixelmap ***mtable);
    ...
    while(command_list->head != NULL) {
        ...
        if(current_command->command[0] == '-') {
            ...
            for(cp = current_command->command + 1; *cp; cp++) {
                ...
                // BRender: Handle options which have an optional argument. We shall decide that
                // BRender: if the optional argument does not exist, we shall set current command
                // BRender: to NULL before continuing.
                if(strchr("s", *cp)) // BRender: Alan's square pixelmap routine. Add other
                                     // BRender: characters here if you want to use optional arguments.
                {
                    // BRender: If the following fails it returns NULL in current command. Ideal as
                    // BRender: we then assume no argument, as we reached the end of the command line,
                    // BRender: or spotted another flag straight after our flag
                ...
                // BRender: Handle options which _require_ an argument
                ...
                switch(*cp) {
                    ...
                    case '?': {
                        ...
                        T_Log("   -S x,y[,<float>]         Scale to new x,y dimensions using optional filter\n"
                              ...
                              "       png    - Portable Network Graphics format  (1/2/4/8/16-bit-per-channel)\n"
                              "       jpg    - JPEG format                       (baseline & progressive)\n"
                              "       pcx    - ZSoft PiCture eXchange format     (1,2,4,8,RLE)\n"
                              "\n"
                        ...
                        for(i = 0; i < PixelmapTypesSize; i++) {
                            T_Log("       %s   \t(%s)\n", PixelmapTypes[i].identifier, PixelmapTypes[i].description);
                        }
                    ...
                    case 'a': {
                        ...
                        for(i = 0; i < nmaps; i++) {
                            if(mtable[i]->type == (alpha_channel == BR_PMT_RGBX_888) ? BR_PMT_RGBA_8888 : BR_PMT_RGBX_888) {
                                ...
                                T_Log("'%s' type already %s\n", mtable[i]->identifier, PixelmapTypes[mtable[i]->type].identifier);
                        }
                    ...
                    case 'I': {
                        ...
                        for(i = 0; i < InputFileTypesSize; i++)
                            if(!BrStrICmp(current_command->command, InputFileTypes[i].name))
                                break;
                    ...
                    case 'O': {
                        ...
                        for(i = 0; i < OutputFileTypesSize; i++)
                            if(!BrStrICmp(current_command->command, OutputFileTypes[i].name))
                                break;
                    ...
                    case 'o': {
                        ...
                        if(ot == TYPE_GUESS && (cp = (char *)strrchr(current_command->command, '.'))) {
                             ...
                             */
                            for(i = 0; i < OutputFileTypesSize; i++)
                                if(!BrStrICmp(OutputFileTypes[i].name, cp + 1))
                                    ot = i;
                        }
                        /* BRender:
                    ...
                    case 'f': {
                        ...
                        T_Log("Current registry data removed (%d pixelmap%s", cbfn_command->nmaps,
                              (cbfn_command->nmaps == 1) ? ")\n" : "s)\n");
                    ...
                    case 'c': {
                        ...
                        cbfn_command->convert_table = BrResAllocate(
                            cbfn_command, sizeof(*cbfn_command->convert_table) * cbfn_command->nmaps, BR_MEMORY_APPLICATION);
                    ...
                    // BRender: -X exclude transparent pixels
                    // BRender:    from remapping / quantising
                    case 'X': {
                    ...
                    case 'P': {
                        ...
                        if(BrStrICmp(arg->command, "RAW") == 0) {
                            /* BRender:
                    ...
                    case 's': {
                        ...
                        // BRender: If there are no command parameters...
                        if(NULL == current_command) {
                            // BRender: set the default
                            cbfn_command->fwidth = 3.0;
                        } else {
                            // BRender: suss out what the parameter contains
                            if((n = T_DecodeCommand(current_command->command, arg_list)) > 1)
                                BR_ERROR0("Too many parameters for -s");

                            arg = (t_cla *)T_RemoveHead(arg_list);
                            // BRender: set the filter size.
                            cbfn_command->fwidth = arg->fvalue;
                        }
                    ...
                    /* BRender:
                                            ...
                                            arg_list = BrResAllocate(res_anchor, sizeof(*arg_list),
                    BR_MEMORY_APPLICATION); T_NewList(arg_list);

                                            cbfn_command = BrResAllocate(res_anchor, sizeof(*cbfn_command),
                    BR_MEMORY_APPLICATION); n = T_DecodeCommand(current_command->command, arg_list);
                    ...
                    case 'F': {
                        ...
                        cbfn_command->map_table = BrResAllocate(
                            cbfn_command, sizeof(*cbfn_command->convert_table) * cbfn_command->nmaps, BR_MEMORY_APPLICATION);
                    ...
                    case 'M': {
                        ...
                        // BRender: Code that generates a combined Mip
                        t_pixelmap_cbfn_info *cbfn_command;
                        t_list               *arg_list;
                        t_cla                *arg;
                        br_int_32             n;
...
#if 0 // BRender: Code that generates seperate maps.
						t_pixelmap_cbfn_info *cbfn_command;
                    ...
                    // BRender: New remapper where you can weight some pixelmaps
                    case 'w': {
                        ...
                        // BRender: Setup a blank supermap table of maximum nparms supermaps
                        smtable = BrMemCalloc(sizeof(*smtable), nparms, BR_MEMORY_APPLICATION);
                        if(NULL == smtable) {
                            BR_ERROR0("Calloc failed. (Probably out of memory)");
                        }

                        // BRender: Now grab the superpixelmaps from wherever they are.
                        while(1) {
                            // BRender: get the command line token
                            arg = (t_cla *)T_RemoveHead(arg_list);
                            if(NULL == arg) {
                                // BRender: stop looking if there are no more arguments
                                break;
                            }

                            // BRender: Check the registry for a file of the same name
                            smtable[nsmaps] = BrMapFind(arg->command);
                            if(NULL == smtable[nsmaps]) {
                                // BRender: Supermap not in registry. Must attempt to load it
                                smtable[nsmaps] = BrPixelmapLoad(arg->command);

                                // BRender: Test for success
                                if(NULL == smtable[nsmaps]) {
                                    // BRender: Give up
                                    BR_ERROR0("Unable to load supermap");
                                }
                            }
                            // BRender: One more supermap in list
                            nsmaps++;
                        }
            ...
            if(it == TYPE_GUESS && (cp = (char *)strrchr(input_name, '.'))) {
                 ...
                 */
                for(i = 0; i < InputFileTypesSize; i++)
                    if(!BrStrICmp(InputFileTypes[i].name, cp + 1))
                        it = i;
            }
            ...
            if(pm->map != NULL)
                T_Log("    Palette '%s' %s (%d, %d)\n", pm->map->identifier, PixelmapTypes[pm->map->type].identifier,
                      pm->map->width, pm->map->height);
        }
    }
...
void put_pmt_rgb_555(br_pixelmap *pm, br_colour colour, int x, int y, int offset, int byte)
{
    ...
    pixel = ((BR_RED(colour) >> 3) << 10) + ((BR_GRN(colour) >> 3) << 5) + ((BR_BLU(colour) >> 3));
...
void put_pmt_rgb_565(br_pixelmap *pm, br_colour colour, int x, int y, int offset, int byte)
{
    ...
    pixel = ((BR_RED(colour) >> 3) << 11) + ((BR_GRN(colour) >> 2) << 5) + ((BR_BLU(colour) >> 3));
