...
{
    (void)command;
    fprintf(stdout, "Usage: 3ds2br <input file name> <options>\n"
                    "       [ -h ]"
...
Bool_t NextToken(FlagParseState_t *fp_state, char *tok_string)
{
    ...
    switch(fp_state->last_flag) {
...
#endif
                    is_ok = ConditionalSetString(&(fp_state->input_filename), tok_string, "Extra input filename %s\n");
                    fp_state->last_flag = cf_none;
                    break;
            }
            break;
        case cf_mod:
            ...
            printf("Mod flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->mod_filename), tok_string, "Extra mod file name %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_mat:
#if SHOW_TOKENS
            printf("Mat flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->mat_filename), tok_string, "Extra mat file name %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_scr:
#if SHOW_TOKENS
            printf("Scr flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->scr_filename), tok_string, "Extra scr file name %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_act:
#if SHOW_TOKENS
            printf("act flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->act_filename), tok_string, "Extra act file name %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_log:
#if SHOW_TOKENS
            printf("log flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->log_filename), tok_string, "Extra log filename option %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_hither:
#if SHOW_TOKENS
...
#endif
            is_ok = ConditionalSetString(&(fp_state->hither_string), tok_string, "Extra hither option %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_yon:
#if SHOW_TOKENS
            printf("yon flag: '%s'\n", tok_string);
#endif
            is_ok               = ConditionalSetString(&(fp_state->yon_string), tok_string, "Extra yon option %s\n");
            fp_state->last_flag = cf_none;
            break;
        case cf_scale:
#if SHOW_TOKENS
            printf("scale flag: '%s'\n", tok_string);
#endif
            is_ok = ConditionalSetString(&(fp_state->scale_string), tok_string, "Extra scale option %s\n");
            fp_state->last_flag = cf_none;
            break;
        default:
...
Bool_t ParseAtFile(FlagParseState_t *fp_state, char *filename)
{
    ...
    while(is_ok && !ferror(fp) && !feof(fp)) {
        if(fgets(line_buffer, line_buffer_length, fp) != NULL) {
            /* BRender: Insert checks for buffer overrun here */
...
Bool_t ParseArgv(FlagParseState_t *fp_state, char *argv[])
{
    ...
    while(is_ok && *argv != NULL) { /* BRender: This is ANSI standard now */
...
int main(int argc, char **argv)
{
    ...
    if(is_ok && fp_state.hither_string != NULL) {
        ...
        /* BRender: FIXME: atof has no error checking.  We could do with some. */
    }
    ...
    if(is_ok) {
        if(fp_state.log_filename != NULL) {
            fplog = fopen(fp_state.log_filename, "w");
            if(fplog == NULL) {
    ...
    if(results == NULL)
        BR_FAILURE1("Cannot convert file '%s'", fp_state.input_filename);
