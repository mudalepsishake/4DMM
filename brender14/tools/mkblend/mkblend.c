        ...
        if(**argv == '-') {
            ...
            for(cp = *argv + 1; *cp; cp++) {
                ...
                switch(*cp) {
                     ...
                     */
                    case '?':
                        fprintf(stderr, "usage: mkblend <palette> [options]\n"
                                        "\n"
...
/*
 */
static void colourOp(float *dest, float *a, float *b)
{
...
void SetBlend(char *table)
{
    ...
    for(a_i = 0, a_cp = SrcPalette->pixels; a_i < PAL_SIZE; a_i++, a_cp++) {
        ...
        for(b_i = 0, b_cp = SrcPalette->pixels; b_i < PAL_SIZE; b_i++, b_cp++) {
            ...
            for(d_i = DestBase, dest_cp = DestPalette->pixels; d_i < (DestBase + DestRange); d_i++, dest_cp++) {
