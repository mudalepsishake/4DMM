...
char buf[1024 * 1024 * 1024]; /* BRenderModern: 1GiB */
...
int main(int argc, char **argv)
{
    ...
    // BRenderModern: gnuplot2d(&graph_cos, stdout);
...
static int gnuplot(size_t n, float *x, float *y_f, float *y_x)
{
    ...
    setvbuf(fp, buf, _IOFBF, sizeof(buf));
    // BRenderModern: fprintf(fp, "set terminal png\n");
    fprintf(fp, "set terminal qt enhanced\n");
    // BRenderModern: fprintf(fp, "set output 'plot.png'\n");
    fprintf(fp, "set xlabel 'x'\n");
    fprintf(fp, "set ylabel 'f(x)'\n");
    fprintf(fp, "plot '-' using 1:2 with lines title 'float', \\\n"
                "     '-' using 1:2 with lines title 'fixed', \\\n"
    ...
    /* BRenderModern: Write the float data. */
    for(size_t i = 0; i < n; ++i) {
        fprintf(fp, "%f %f\n", x[i], y_f[i]);
    }
    fputs("e\n", fp);

    /* BRenderModern: Write the fixed data. */
    for(size_t i = 0; i < n; ++i) {
        fprintf(fp, "%f %f\n", x[i], y_x[i]);
    }
    fputs("e\n", fp);

    /* BRenderModern: Write the diff data. */
    for(size_t i = 0; i < n; ++i) {
