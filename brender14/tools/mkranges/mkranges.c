...
int main(int argc, char **argv)
{
    ...
    while(fgets(line_buffer, BR_ASIZE(line_buffer), infile)) {
        ...
        n = sscanf(line_buffer,
                   /* BRender:  Range Ambient  Diffuse  Specular Cut */
                   "%d,%d %d,%d,%d %d,%d,%d %d,%d,%d %f", &base, &range, &ar, &ag, &ab, &dr, &dg, &db, &sr, &sg, &sb, &cut);
        ...
        for(n = 0; n < range; n++, cp += palette->row_bytes)
            *(br_colour *)cp = MakeColour(BR_COLOUR_RGB(ar, ag, ab), BR_COLOUR_RGB(dr, dg, db),
                                          BR_COLOUR_RGB(sr, sg, sb), cut, n / (float)range);
    }
...
static br_colour MakeColour(br_colour a, br_colour d, br_colour s, float cut, float level)
{
    ...
    if(level < cut) {
        ...
        return BR_COLOUR_RGB((1.0 - level) * BR_RED(a) + level * BR_RED(d), (1.0 - level) * BR_GRN(a) + level * BR_GRN(d),
                             (1.0 - level) * BR_BLU(a) + level * BR_BLU(d));
    } else {
        level = (level - cut) / (1.0 - cut);

        return BR_COLOUR_RGB((1.0 - level) * BR_RED(d) + level * BR_RED(s), (1.0 - level) * BR_GRN(d) + level * BR_GRN(s),
