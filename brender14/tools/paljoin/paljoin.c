...
void main(int argc, char *argv[])
{
    ...
    // BRender: Give user options list if inappropriate argc
    if(argc != 3) {
    ...
    while(!feof(fileHandle)) {
        ...
        // BRender: Read in line from file
        inStr[0] = '\n'; // BRender: make sure it can handle empty lines
        fgets(inStr, MAX_LINE, fileHandle);
        // BRender: Break it into parameters
        ...
        // BRender: Skip comments and blank lines
        if(!((*inFileName == '#') || (*inFileName == ';') || (*inFileName == '\n'))) {

            // BRender: Pre-process parameters
            ...
            // BRender: Tell the user what is happening
            printf("Processing line %d: %s %d %d %d\n", lineCount, inFileName, sourceBase, range, destinationBase);
            if(sourceBase + range > MAX_ENTRIES)
                fprintf(stderr, "\tSourceIndexBase+Range>256 will not read past end of palette\n");
            if(destinationBase + range > MAX_ENTRIES)
                fprintf(stderr, "\tDestinationIndexBase+Range>256 will not write past end of palette\n");

            // BRender: reset origin

            sourceMap->origin_x = 0;
            sourceMap->origin_y = 0;

            // BRender: Do the real work
            for(y = 0; (y < range) && (y + sourceBase < MAX_ENTRIES); y++) {
                colour = BrPixelmapPixelGet(sourceMap, 0, y + sourceBase);
                if(destinationBase + y < MAX_ENTRIES)
                    BrPixelmapPixelSet(destinationMap, 0, y + destinationBase, colour);
            }
        }
    }

    // BRender: Write out the results
    if(!BrPixelmapSave(argv[2], destinationMap)) {
        fprintf(stderr, "Failed to write output file");
    }
    printf("Successfully created %s.\n", argv[2]);

    // BRender: Clean up and exit
